...
//                            isbean = parReurn.isBean();//样例代码
	...
	private static void saveWriteFile(String fileName, String content) {
		//FileWriter fw = null;
		...
		try {
           ...
           /* fw = new FileWriter(fileName, true);
			...
			logger.error("生成接口文件失败!" + ioe.getMessage());
		} finally {
			if (writer != null) {
				try {
					writer.close();
	...
	private static JSONArray params(List<ParamsBean> apiParamsList) {
