...
public class DMLDataQuery {
	...
	@Method(desc = "获取加工下表信息列表", logicStep = "获取加工下表信息列表")
	@Param(name = "category_id", desc = "加工分类id,该值唯一", range = "long类型")
	@Param(name = "jobExecuteState", desc = "JobExecuteState作业执行状态", range = "如果执行状态不为null,查询指定状态的表,如果为null,查询所有表")
	@Return(desc = "返回值说明", range = "返回值取值范围")
	public static List<Map<String, Object>> getDMLTableInfos(long category_id, JobExecuteState jobExecuteState) {
		//初始化查询sql
		SqlOperator.Assembler asmSql = SqlOperator.Assembler.newInstance();
		asmSql.clean();
		asmSql.addSql("select * from " + Dm_datatable.TableName + " dm" +
				" join " + Dtab_relation_store.TableName + " dtab_rs on dm.datatable_id=dtab_rs.tab_id" +
				" where dm.category_id=?").addParam(category_id);
		if (null != jobExecuteState) {
			asmSql.addSql(" and dtab_rs.is_successful=?").addParam(jobExecuteState.getCode());
		}
		return Dbo.queryList(asmSql.sql(), asmSql.params());
	}
