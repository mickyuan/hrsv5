...
public class DataTableUtil {
	...
	public static List<Map<String, Object>> getColumnByTableName(DatabaseWrapper db, String table_name) {
		...
		if (!column_list.isEmpty()) {
			...
			//DQC
			asmSql.clean();
			Optional<Dq_index3record> di3_option = SqlOperator.queryOneObject(db, Dq_index3record.class,
					"SELECT * FROM " + Dq_index3record.TableName + " WHERE table_name = LOWER(?)", table_name);
			if (di3_option.isPresent()) {
				Dq_index3record di3 = di3_option.get();
