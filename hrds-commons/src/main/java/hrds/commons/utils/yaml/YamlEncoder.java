...
import hrds.commons.utils.yaml.wrapper.CollectionWrapper;
import hrds.commons.utils.yaml.wrapper.MapWrapper;
...
/**
 * YamlEncoder - The usage of YamlEncoder mirrors that of java.beans.XMLEncoder. You create an
 * encoder, make some calls to writeObject, and then close the encoder. In most cases you may find
 * it is not necessary to access this class directly as {@link Yaml} contain methods for the most
 * common usages. The utility functions that were previously in this class are now in {@link Yaml}.
 */
public class YamlEncoder {
  ...
  void writeMap(MapWrapper map, String indent, Class expectedType) {
    if ((isMinimalOutput() && expectedType == map.getType())
        || expectedType == HashMap.class
  ...
  void writeCollection(CollectionWrapper col, Class expectedType, String indent) {
    if (col.size() > 0) {
      if ((isMinimalOutput() && expectedType == col.getType())
          || col.getType() == ArrayList.class
  ...
  /**
   * Closes this YamlEncoder instance. This must be done after a write sequence for the write to be
   * effective.
   */
  public void close() {
    out.close();
