...
public class EtlJobUtil {
    ...
    public static void saveJob() {
        saveJob("", DataSourceType.DCL, ETL_SYS_CD, "", CollectType.FtpCaiJi);
        saveJob("", DataSourceType.DCL, ETL_SYS_CD, "", CollectType.DuiXiangWenJianCaiJi);
        saveJob("", DataSourceType.DCL, ETL_SYS_CD, "", CollectType.WenJianCaiJi);
        saveJob("", DataSourceType.DCL, ETL_SYS_CD, "", CollectType.DBWenJianCaiJi);
        saveJob("", DataSourceType.DCL, ETL_SYS_CD, "", CollectType.ShuJuKuCaiJi);
    ...
    public static int saveJob(String pkId, DataSourceType dataSourceType, String etl_sys_cd, String sub_sys_cd,
        ...
        try (DatabaseWrapper db = new DatabaseWrapper()) {
            ...
            if (dataSourceType == DataSourceType.DCL) {
                if (collectType == CollectType.FtpCaiJi) {
                    throw new BusinessException("暂未实现!");
                }
                if (collectType == CollectType.DuiXiangWenJianCaiJi) {
                    throw new BusinessException("暂未实现!");
                }
                if (collectType == CollectType.WenJianCaiJi) {
                    throw new BusinessException("暂未实现!");
                }
                if (collectType == CollectType.ShuJuKuCaiJi || collectType == CollectType.DBWenJianCaiJi) {
                    throw new BusinessException("暂未实现!");
                }
            }
            //DPL层
