...
public class UnstructuredFileCollectAction extends BaseAction {
	...
	public void updateFileCollect(File_collect_set file_collect_set) {
		...
		//更新之后没有重新发送之前默认为false TODO 这里设置发送状态为(0:否) 会导致任务在编辑未完成时查询不到该任务信息
//		file_collect_set.setIs_sendok(IsFlag.Fou.getCode());
		//1.更新时,根据fcs_name查询非结构化任务名称是否与其他采集任务名称重复
		File_collect_set fcs_info = Dbo.queryOneObject(File_collect_set.class,
			"SELECT * FROM " + File_collect_set.TableName + " WHERE fcs_id=?", file_collect_set.getFcs_id())
			.orElseThrow(() -> (new BusinessException("查询文件采集任务信息的SQL失败!")));
	...
	@Param(name = "execute_type", desc = "执行类型", range = "不可为空,{execute_immediately:立即执行,execute_etl:作业调度执行}")
	public void executeJob(long fcs_id, String execute_type) {
		//接收参数，执行作业，立即启动这里应该会有超时问题，启动作业后是否马上给返回值，后面的作业调度，待讨论
		Result result = Dbo.queryResult("SELECT t1.*,t2.agent_name,t3.source_id,t3.datasource_name,t4.dep_id " +
			" FROM " + File_collect_set.TableName + " t1 " +
			" LEFT JOIN " + Agent_info.TableName + " t2 ON t1.agent_id = t2.agent_id " +
			" LEFT JOIN " + Data_source.TableName + " t3 ON t3.source_id = t2.source_id " +
			" LEFT JOIN " + Sys_user.TableName + " t4 ON t4.user_id = t2.user_id " +
		...
		//1.根据前端传过来的agent_id和执行类型获取需要访问的url
		String url;
		if ("execute_etl".equals(execute_type)) {
			url = AgentActionUtil.getUrl(agent_id, getUserId(), AgentActionUtil.EXECUTEFILECOLLECT);
		} else if ("execute_immediately".equals(execute_type)) {
			url = AgentActionUtil.getUrl(agent_id, getUserId(), AgentActionUtil.EXECUTEFILECOLLECTIMMEDIATELY);
		} else {
			throw new BusinessException("执行类型不合法! execute_type: "
				+ execute_type + "see(execute_immediately:立即执行,execute_etl:作业调度执行)");
		}
		//调用工具类方法给agent发消息，并获取agent响应
		//2.调用远程Agent后端代码获取Agent服务器上文件夹路径
		HttpClient.ResponseValue resVal = new HttpClient()
