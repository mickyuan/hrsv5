	...
	public List<Map<String, Object>> getTbStoDestByColSetId(long colSetId) {
		...
		for (Object tableId : tableIds) {
			...
			if (list.isEmpty()) {
				...
				Assembler assembler = Assembler.newInstance().addSql("select t1.hyren_name from table_storage_info t1" +
						" join dtab_relation_store t2 on t1.storage_id = t2.tab_id where t1.table_id = ? ");
