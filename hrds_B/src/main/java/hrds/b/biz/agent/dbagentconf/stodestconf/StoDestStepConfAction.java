...
public class StoDestStepConfAction extends BaseAction {
	 ...
	 */
	@Method(desc = "根据数据库设置ID获得定义存储目的地页面初始化信息",
		logicStep = "1、根据colSetId进行查询，查询出前端需要展示的数据"
	...
	public Map<String, Object> getInitInfo(long colSetId) {
		...
		Result result = Dbo.queryResult(
			" select ti.table_id, ti.table_name, ti.table_ch_name, tsi.is_zipper, tsi.storage_type," +
				" tsi.storage_time," + " ded.data_extract_type, '0' as destflag ,tsi.hyren_name" +
				...
				" where ti.database_id = ? ORDER BY ti.table_name",
			colSetId);
		// 2、根据colSetId在table_info中获取该采集任务所有的采集表id集合
		List<Object> list = Dbo.queryOneColumnList(
			...
			for (int j = 0; j < result.getRowCount(); j++) {
				...
				if (tableIdFromTI.equals(tableIdFromResult)) {
					long count = Dbo.queryNumber(
						"select count(1) from " + Table_storage_info.TableName + " where table_id = ?", tableIdFromTI)
		...
		Map<String, Object> collectMapData = Dbo.queryOneObject(
			"SELECT t3.datasource_number,t4.classify_num FROM " + Database_set.TableName + "" +
				" t1 JOIN " + Agent_info.TableName + " t2 ON t1.agent_id = t2.agent_id" +
				" JOIN " + Data_source.TableName + " t3 ON t2.source_id = t3.source_id" +
				" join " + Collect_job_classify.TableName + " t4 on t1.classify_id = t4.classify_id" +
				" WHERE t1.database_id = ?", colSetId);
		collectMapData.put("storageTableData", result.toList());
		// 6、将查询到的信息返回前端
		return collectMapData;
	}

	@Method(desc = "根据数据库设置ID获取当前数据库直连采集任务下所有抽取及入库的表的存储目的地ID",
		logicStep = "1、根据数据库设置ID获取当前数据库采集任务所有抽取并入库的表ID"
			+ "2、遍历结果集，根据表ID获取该表定义的存储目的地"
			+ "3、key为tableId,value为抽取及入库的表ID，key为dslIds,value为该表定义的存储目的地")
	@Param(name = "colSetId", desc = "数据库设置ID，源系统数据库设置表主键，数据库对应表外键", range = "不为空")
	@Return(desc = "查询结果集",
		range = "不为空，key为tableId,value为抽取及入库的表ID，"
	...
	public List<Map<String, Object>> getTbStoDestByColSetId(long colSetId) {
		...
		List<Object> tableIds = Dbo.queryOneColumnList(
			"select ti.table_id from " + Table_info.TableName + " ti" +
				" join " + Data_extraction_def.TableName + " ded on ti.table_id = ded.table_id" +
				" where ti.database_id = ? and ded.data_extract_type = ? ORDER BY ti.table_name",
			colSetId, DataExtractType.YuanShuJuGeShi.getCode());
		if (tableIds.isEmpty()) {
			throw new BusinessException("未获取到数据库采集表");
		}
		...
		for (Object tableId : tableIds) {
			...
			List<Object> list = Dbo.queryOneColumnList(
				"SELECT drt.dsl_id FROM " + Dtab_relation_store.TableName + " drt" +
					" WHERE drt.tab_id = (" +
					"   SELECT storage_id FROM " + Table_storage_info.TableName +
					"   WHERE table_id = ?" +
					" ) AND drt.data_source = ?",
			...
			if (list.isEmpty()) {
				...
				Assembler assembler = Assembler.newInstance()
					.addSql("select t1.hyren_name,t1.storage_type from " + Table_storage_info.TableName + " t1" +
						" join " + Dtab_relation_store.TableName
						+ " t2 on t1.storage_id = t2.tab_id where t1.table_id = ?");
	...
	@Return(desc = "查询结果集",
		range = "不为空"
			...
			+ "如: 数据库名称:PGSQL 数据库用户名:test")
	public Result getStoDestDetail(long dslId) {
		// 1、根据存储配置主键去数据库中查询存储目的地详细信息
		return Dbo.queryResult(" select storage_property_key, storage_property_val" +
			" from " + Data_store_layer_attr.TableName + " where dsl_id = ?", dslId);
	}
	 ...
	 * */
	@Method(desc = "对仅做数据抽取的表回显定义好的存储目的地",
		logicStep = "1、校验该表定义的数据抽取信息是否存在，之所以这样校验是因为配置抽取属性是上一个页面的职责，"
	...
	public Result getStoDestForOnlyExtract(long tableId) {
		...
		long count = Dbo.queryNumber("select count(1) from " + Data_extraction_def.TableName +
			...
			tableId, DataExtractType.ShuJuKuChouQuLuoDi.getCode())
			.orElseThrow(() -> new BusinessException("SQL查询错误"));
		if (count != 1) {
			throw new BusinessException("获取该表数据抽取信息异常");
		}
		// 2、查询数据抽取定义表，获取数据落地目录并返回
		return Dbo.queryResult("select plane_url from " + Data_extraction_def.TableName
				+ " where table_id = ? and data_extract_type = ?",
			tableId, DataExtractType.ShuJuKuChouQuLuoDi.getCode());
	}
	 ...
	 * TODO 在没有完全确定之前，这个接口暂时保留
	 * */
	@Method(desc = "对仅做数据抽取的表保存定义好的存储目的地",
		logicStep = "1、使用tableId进行校验，判断该表是否定义过数据抽取信息，且数据抽取方式为仅抽取"
			+ "2、若不存在，向前端抛异常"
	...
	public void saveStoDestForOnlyExtract(long tableId, String stoDest) {
		...
		long count = Dbo.queryNumber(
			"select count(1) from " + Data_extraction_def.TableName + " where table_id = ? and data_extract_type = ?",
			tableId, DataExtractType.ShuJuKuChouQuLuoDi.getCode())
			.orElseThrow(() -> new BusinessException("SQL查询错误"));
		// 2、若不存在，向前端抛异常
		if (count != 1) {
		...
		DboExecute.updatesOrThrow("保存存储目的地失败",
	 ...
	 * */
	@Method(desc = "根据tableId回显该表选择的存储目的地和系统中配置的所有存储目的地",
		logicStep = "1、获取所有存储目的地信息，并且追加一列usedflag，固定值为'0'，表示默认所有存储目的地没有被这个当前表所使用"
	...
	@Return(desc = "查询结果集",
		range = "不为空，注意每条数据的usedFlag字段，true表示该表配置了该存储目的地，"
	...
	public Map<String, Object> getStoDestByTableId(long tableId) {
		...
		// 3、尝试获取该表定义好的存储目的地，如果之前定义过，那么就能获取到数据，否则就获取不到
		Result tbStoRela = Dbo.queryResult(
			"SELECT t2.dsl_id,t1.hyren_name FROM " + Table_storage_info.TableName + " t1" +
				" JOIN " + Dtab_relation_store.TableName + " t2 ON  t1.storage_id = t2.tab_id" +
				" WHERE t1.table_id = ? AND t2.data_source = ?",
			tableId, StoreLayerDataSource.DB.getCode());
		// 4、如果获取不到，说明之前该表未定义过存储目的地，则直接返回结果集
	...
	public Result getStorageData() {
		// 1、获取所有存储目的地信息，并且追加一列usedflag，固定值为0，表示默认所有存储目的地没有被这个当前表所使用
		Result result = Dbo.queryResult(
			"SELECT dsl_id, dsl_name, store_type, '0' as usedflag FROM " + Data_store_layer.TableName);
		// 2、如果结果集为空，表示系统中没有定义存储目的地
		if (result.isEmpty()) {
	...
	@Method(desc = "根据存储目的地ID获取选择列画面需要展示的表头信息",
		logicStep = "1、无论是选择什么存储目的地，都需要展示列名和列中文名这两列"
	...
	public Map<String, String> getColumnHeader(long dslId) {
		...
		// 2、根据存储目的地ID去数据存储附加信息表中获取需要额外展示的列
		List<Object> list = Dbo.queryOneColumnList(
			"select dsla_storelayer from " + Data_store_layer_added.TableName + " where dsl_id = ?", dslId);
		if (!list.isEmpty()) {
			for (Object obj : list) {
				// 3、将列的code值转换成枚举项的value放入map中
	...
	@Method(desc = "根据存储目的地ID获取选择列画面需要的附加属性信息ID",
		logicStep = "" + "1、根据存储目的地ID去数据存储附加信息表中获取附加属性信息ID和附加属性信息" + "2、封装成Map集合返回前端")
	@Param(name = "dslId", desc = "数据存储层配置表主键，数据存储附加信息表外键", range = "不为空")
	@Return(desc = "Map集合",
		range = "key为附加信息code，value为附加信息ID，"
	...
	public Map<String, Long> getDataStoreLayerAddedId(long dslId) {
		...
		Result result = Dbo.queryResult(
			"select dslad_id, dsla_storelayer from " + Data_store_layer_added.TableName + " where dsl_id = ?", dslId);
		if (result.isEmpty()) {
			return storeAddedId;
		}
		for (int i = 0; i < result.getRowCount(); i++) {
	...
	@Method(desc = "根据表ID获取该表所有的列存储信息",
		logicStep = "1、根据表ID查询出该表所有采集列的列名和列中文名(结果集1)"
	...
	public Result getColumnStoInfo(long tableId, long dslId) {
		...
		Result resultOne = Dbo.queryResult(
			"select tc.column_id, tc.column_name, tc.column_ch_name from " + Table_column.TableName + " tc" +
				" where tc.table_id = ? and tc.is_get = ?",
			tableId, IsFlag.Shi.getCode());
		if (resultOne.isEmpty()) {
			throw new BusinessException("未找到属于该表的字段");
		}
		// 2、根据存储目的地ID获取附加信息(结果集2)
		Result resultTwo = Dbo.queryResult(
			"select dsla.dsla_storelayer from " + Data_store_layer_added.TableName + " dsla" +
				" join " + Data_store_layer.TableName + " dsl on dsla.dsl_id = dsl.dsl_id" +
				" where dsl.dsl_id = ?", dslId);
		...
		if (!resultTwo.isEmpty()) {
			for (int i = 0; i < resultTwo.getRowCount(); i++) {
				for (int j = 0; j < resultOne.getRowCount(); j++) {
					// 这里给数据存储附加信息设置一个默认值0
					resultOne.setObject(j, StoreLayerAdded.ofValueByCode(resultTwo.getString(i, "dsla_storelayer")),
						IsFlag.Fou.getCode());
				}
			}
		} else {
		...
		Result resultThree = Dbo.queryResult(
			"select csi.col_id column_id, dsla.dsla_storelayer, csi_number" +
				" from " + Dcol_relation_store.TableName + " csi" +
				" left join " + Data_store_layer_added.TableName + " dsla on dsla.dslad_id = csi.dslad_id" +
				" where csi.col_id in (select column_id from " + Table_column.TableName + " where table_id = ?)" +
				" and dsla.dsl_id = ? AND  csi.data_source = ? ",
			tableId, dslId, StoreLayerDataSource.DB.getCode());
		for (int i = 0; i < resultThree.getRowCount(); i++) {
			...
			for (int j = 0; j < resultOne.getRowCount(); j++) {
				...
				if (columnIdFromCSI == columnIdFromTC) {
					resultOne.setObject(j, StoreLayerAdded.ofValueByCode(resultThree.getString(i, "dsla_storelayer")),
						IsFlag.Shi.getCode());
	...
	public void saveColStoInfo(String colStoInfoString, long tableId) {
		...
		// 2、在每保存一个字段的存储目的地前，先尝试在column_storage_info表中删除该表所有列的信息，不关心删除的数据
		Dbo.execute("delete from " + Dcol_relation_store.TableName +
			" where col_id in (select column_id from " + Table_column.TableName + " where table_id = ? )" +
			" AND data_source = ?", tableId, StoreLayerDataSource.DB.getCode());
		if (!colStoParams.isEmpty()) {
			...
			for (ColStoParam param : colStoParams) {
				...
				for (long dsladId : dsladIds) {
					...
					List<Object> list = Dbo.queryOneColumnList(
						"select dsl.store_type from " + Data_store_layer.TableName + " dsl, "
	...
	public void updateColumnZhName(String columnString) {
		...
		for (int i = 0; i < tableColumns.size(); i++) {
			...
			DboExecute.updatesOrThrow("保存第" + (i + 1) + "个字段的中文名失败",
	 ...
	 * */
	@Method(desc = "在配置表存储信息时，更新表中文名和表名",
		logicStep = "1、将传过来的json串反序列化为List集合" +
			"2、对集合的长度进行校验，如果集合为空，抛出异常" +
			"3、遍历集合，更新每张表的中文名和表名")
	@Param(name = "tableString",
		desc = "待更新的字段信息，json数组",
	...
	public void updateTableName(String tableString) {
		...
		for (int i = 0; i < tableInfos.size(); i++) {
			...
			DboExecute.updatesOrThrow("保存第" + i + "张表名称信息失败",
				"update " + Table_info.TableName + " set table_name = ?, table_ch_name = ? where table_id = ?",
				tableInfo.getTable_name(), tableInfo.getTable_ch_name(), tableInfo.getTable_id());
		}
	}

	@Method(desc = "校验保存表存储配置信息时各个字段的合法性",
		logicStep = "1、校验保存表存储配置信息时，必须关联表"
...
//				}
	...
	@Method(desc = "保存表存储属性配置，仅保存抽取方式为<抽取并入库>的表",
		logicStep = "1、将tbStoInfoString反序列化为List集合，这个集合中的内容是用来保存进入表存储信息表"
	...
	public long saveTbStoInfo(String tbStoInfoString, long colSetId, String dslIdString) {
		...
		for (Table_storage_info storageInfo : tableStorageInfos) {
			...
			long count = Dbo.queryNumber(
				...
				storageInfo.getTable_id()).orElseThrow(() -> new BusinessException("SQL查询错误"));
			if (count == 1) {
				 ...
				 * */
				Dbo.execute("delete from " + Dtab_relation_store.TableName +
						" where tab_id in (select storage_id from " + Table_storage_info.TableName + " where table_id=?)" +
						" AND data_source = ?",
				...
				DboExecute.deletesOrThrow("删除表存储信息异常，一张表入库信息只能在表存储信息表中出现一条记录",
					"delete from " + Table_storage_info.TableName + " where table_id = ?", storageInfo.getTable_id());
			}
			// 5-2、对待保存的数据设置主键等信息
...
//			5-7: 检查每个最终的表名是否被使用过,如果有则抛出异常信息
			...
			//5-7 为了防止上次选择了拉链,此次编辑时取消了...则将字段的拉链信息更新为否
			if (IsFlag.Fou == IsFlag.ofEnumByCode(storageInfo.getIs_zipper())) {
				Dbo.execute("UPDATE " + Table_column.TableName + " SET is_zipper_field = ? WHERE table_id = ?",
					IsFlag.Fou.getCode(), storageInfo.getTable_id());
			}
			// 5-8、保存表存储信息
	...
	private List<Object> getStoreLayerDataByOracle() {
		...
		return Dbo.queryOneColumnList("select dsl_id from " + Data_store_layer_attr.TableName +
				" where storage_property_key = ? AND (storage_property_val = ? OR storage_property_val = ?)",
			StorageTypeKey.database_type, DatabaseType.Oracle10g.getCode(), DatabaseType.Oracle9i.getCode());
	}
