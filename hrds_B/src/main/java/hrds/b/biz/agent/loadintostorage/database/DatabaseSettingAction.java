package hrds.b.biz.agent.loadintostorage.database;
...
@DocClass(desc = "数据库采集管理类", author = "Mr.Lee", createdate = "2020-08-17 14:32")
public class DatabaseSettingAction extends BaseAction {

	@Method(desc = "新增数据库采集信息", logicStep = "1: 根据用户和Agent信息进行关联查询上次为配置完成的任务信息,如果没有则说明是新增")
	...
	public Result getInitDatabase(long source_id, long agent_id) {
		...
		return Dbo.queryResult(
			...
			getUserId(),
			source_id,
			agent_id, CollectType.ShuJuKuCaiJi.getCode());
	}

	@Method(
		desc = "根据数据库采集任务ID查询数据库采集信息",
		logicStep =
			""
	...
	public Long saveDatabaseInfo(Database_set databaseSet) {
		//1: 校验实体每个必须字段的数据不能为空
	...
	@Param(name = "databaseSet", desc = "贴源数据的实体信息", range = "不可以为空", isBean = true)
	@Return(desc = "返回此次保存后生成的任务ID", range = "不可为空")
	public Long updateDatabaseInfo(Database_set databaseSet) {
	...
	private void verifyDatabaseSetEntity(Database_set databaseSet) {
		...
		Validator.notBlank(databaseSet.getDatabase_type(), "数据库类型不能为空");
...
//		Validator.notBlank(databaseSet.getDatabase_number(), "作业编号不为能空，并且长度不能超过10");
		// 4、校验数据库驱动不能为空
		Validator.notBlank(databaseSet.getDatabase_drive(), "数据库驱动不能为空");
//		// 5、校验数据库名称不能为空
		Validator.notBlank(databaseSet.getDatabase_name(), "数据库名称不能为空");
//		// 6、校验数据库IP不能为空
		Validator.notBlank(databaseSet.getDatabase_ip(), "数据库IP地址不能为空");
//		// 7、校验数据库端口号不能为空
		Validator.notBlank(databaseSet.getDatabase_port(), "数据库端口号不能为空");
		// 8、校验用户名不能为空
		Validator.notBlank(databaseSet.getUser_name(), "数据库用户名不能为空");
		// 9、校验数据库密码不能为空
		Validator.notBlank(databaseSet.getDatabase_pad(), "数据库密码不能为空");
		// 10、校验JDBCURL不能为空
		Validator.notBlank(databaseSet.getJdbc_url(), "数据库连接URL不能为空");
		// 11、校验agent_id不能为空
