...
//			colSetId);
		...
		EtlJobUtil.setDefaultEtlResource(Dbo.db(), etl_sys_cd);

		// 获取作业资源关系信息
		List<String> jobResource = EtlJobUtil.getJobResource(Dbo.db(), etl_sys_cd);

		// 获取抽数关系依赖信息
		List<Object> relationEtl = getRelationEtl(source_id);

		// 先获取当前工程,任务下的作业名称
		List<String> etlJobList = EtlJobUtil.getEtlJob(Dbo.db(), etl_sys_cd, sub_sys_cd);
      ...
      */
			 ...
			 */
			EtlJobUtil.setEtl_job_resource_rela(Dbo.db(), etl_sys_cd, etl_job_def, jobResource);
