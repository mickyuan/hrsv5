...
public class CollTbConfStepAction extends BaseAction {
  ...
  @Method(desc = "根据数据库设置ID获得用户自定义抽取SQL", logicStep = "" + "1、根据colSetId在table_info表中查询数据并返回")
  @Param(name = "colSetId", desc = "数据库设置ID,源系统数据库设置表主键，数据库对应表外键", range = "不为空")
  ...
  public List<Table_info> getAllSQLs(long colSetId) {
    ...
    return Dbo.queryList(
        Table_info.class,
        " SELECT table_id, table_name, table_ch_name, sql "
            + " FROM "
            + Table_info.TableName
  ...
  public Result getSQLInfoByColSetId(long colSetId) {
    ...
    // 3、根据数据库设置ID在数据库采集对应表中获取SQL过滤，是否并行抽取，分页SQL，数据总量，分页并行数，每天数据增量返回给前端
    return Dbo.queryResult(
        "select table_id, table_name, sql, is_parallel, page_sql, table_count, pageparallels, dataincrement "
            + " from "
            + Table_info.TableName
            + " where database_id = ? and is_user_defined = ?",
        colSetId,
  ...
  private List<Table_column> getColumnInfoByTableName(
    ...
    // 2、封装数据，调用方法和agent交互，获取列信息
    String respMsg =
        SendMsgUtil.getColInfoByTbName(agentId, getUserId(), databaseInfo, tableName, methodName);
    // 3、将列信息反序列化为Json数组
  ...
  public long saveCollTbInfo(
    ...
    if (tableInfoString != null && collTbConfParamString != null) {
      ...
      for (int i = 0; i < tableInfoString.length; i++) {
        ...
        // 已经存在，且有可能经过了修改
        // 5、如果是新增采集表
        if (StringUtil.isBlank(tableInfo.getTable_id().toString())) {
          // 5-1、生成table_id，并存入Table_info对象中
          tableInfo.setTable_id(PrimayKeyGener.getNextId());
          tableInfo.setTi_or(DEFAULT_TABLE_CLEAN_ORDER.toJSONString());
          // 5-2、保存Table_info对象
  ...
  @Param(
      name = "columnTypeName",
      desc = "字符串形式的数据库类型，通过调用ResultSetMetaData.getColumnTypeName()得到",
      range = "不为null")
  @Param(
      name = "precision",
      desc = "对于数字类型，precision表示的是数字的精度，对于字符类型，这里表示的是长度，" + "调用ResultSetMetaData.getPrecision()得到",
      range = "不限")
  @Param(
      name = "scale",
      desc = "列数据类型小数点右边的指定列的位数，调用ResultSetMetaData.getScale()得到",
  ...
  private String getColTypeAndPreci(
      int columnType, String columnTypeName, int precision, int scale) {
    /*
  ...
  private void updateTableId(long newID, long oldID) {
    ...
    if (!tableCleanIdList.isEmpty()) {
      ...
      for (int j = 0; j < tableCleanIdList.size(); j++) {
        ...
        if (j != tableCleanIdList.size() - 1) tableCleanBuilder.append(",");
      }
    ...
    if (!colMergeIdList.isEmpty()) {
      ...
      for (int j = 0; j < colMergeIdList.size(); j++) {
        ...
        if (j != colMergeIdList.size() - 1) colMergeBuilder.append(",");
      }
  ...
  private void saveCustomSQLColumnInfoForAdd(Table_info tableInfo, long colSetId, long userId) {
    ...
    String respMsg =
        SendMsgUtil.getCustColumn(
            agentId, getUserId(), databaseInfo, tableInfo.getSql(), methodName);
    // 3、将列信息反序列化为List集合
