...
public class AgentInfoAction extends BaseAction {
	...
	@Param(name = "datasource_name", desc = "数据源名称", range = "不为空，10位数字")
	...
	public void deleteAgent(Long agent_id, String agent_type) {
		...
		// 2.删除前查询此agent是否已部署
		if (Dbo.queryNumber("select count(1) from " + Agent_down_info.TableName + " where agent_id=?", agent_id)
				.orElseThrow(() -> new BusinessException("sql查询错误！")) > 0) {
			// 此agent已部署不能删除
