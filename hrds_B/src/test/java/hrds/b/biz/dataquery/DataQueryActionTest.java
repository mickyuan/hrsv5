...
public class DataQueryActionTest extends WebBaseTestCase {
	...
	//测试数据的用户ID
	private static final long USER_ID = 5000L + THREAD_ID;
	//测试数据的数据源ID
	private static final long SOURCE_ID = 5000000000L + THREAD_ID;
	//测试数据的部门ID
	private static final long DEP_ID = 5000000000L + THREAD_ID;
	//测试数据的采集Agent初始化ID，用来初始化数据
	private static final long AGENT_ID = 5000000000L + THREAD_ID;
	//测试数据的文件采集任务初始化ID，用来初始化数据
	private static final long FCS_ID = 5000000000L + THREAD_ID;
	//测试数据的文件id
	private static final String FILE_ID = "99999999999" + THREAD_ID;
	//测试数据 data_auth 的ID
	private static final long DA_ID = 5000000000L + THREAD_ID;
	//测试数据 search_info 的ID
	private static final long SI_ID = 5000000000L + THREAD_ID;
	//测试数据 user_fav 的ID
	private static final long FAV_ID = 5000000000L + THREAD_ID;
	...
	public void getFileCollectionTask() {
		...
		for (int i = 0; i < ar.getDataForResult().getRowCount(); i++) {
			...
			assertThat(ar.getDataForResult().getLong(i, "fcs_id"), is(FCS_ID));
			assertThat(ar.getDataForResult().getLong(i, "user_id"), is(USER_ID));
	...
	public void getFileClassifySum() {
		...
		assertThat(ar.isSuccess(), is(true));
		//1-2.正确数据访问结果校验
		assertThat(ar.getDataForResult().getString(0, "file_type"), is("文档"));
		assertThat(ar.getDataForResult().getInt(0, "sum_num"), is(5));
	}

	@Method(desc = "7天内文件采集统计测试方法，默认统计7天", logicStep = "如果查询天数小于1天则显示默认7天，查询条数大于30天则显示30天，否则取传入的查询天数")
	@Test
	public void getSevenDayCollectFileSum() {
		...
		ar = JsonUtil.toObjectSafety(bodyString, ActionResult.class).orElseThrow(() -> new BusinessException(
				"获取返回的ActionResult信息失败!"));
		assertThat(ar.isSuccess(), is(true));
		assertThat(ar.getDataForResult().getRowCount(), is(5));
	}
...
//				.orElseThrow(() -> new BusinessException("获取返回的ActionResult信息失败!"));
//		assertThat(ar.isSuccess(), is(true));
//		//2-1.文件id不存在
//		bodyString = new HttpClient()
//				.addData("fileId", "-" + FILE_ID)
