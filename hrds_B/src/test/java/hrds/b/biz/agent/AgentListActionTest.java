...
//			objectCollect.setRun_way(runWay);
			...
			objectCollect.setFile_suffix("txt");
...
//	}
	...
	private void buildTestDataForSendTask() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			for (int i = 0; i < 2; i++) {
				...
				storageInfo.setHyren_name("test_table_name" + id);
	...
	public void after() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			SqlOperator.execute(db, "delete from " + Table_column.TableName + " WHERE table_id in (?,?,?)", TABLE_ID,
					SYS_USER_TABLE_ID, CODE_INFO_TABLE_ID);
			//3-1-2、删除table_info(表对应字段)测试数据
			SqlOperator.execute(db, "delete from " + Table_info.TableName + " WHERE table_id in (?,?,?)", TABLE_ID,
					SYS_USER_TABLE_ID, CODE_INFO_TABLE_ID);
			//3-2、删除非结构化文件采集测试数据
			SqlOperator.execute(db, "delete from " + File_source.TableName + " WHERE agent_id = ?", NON_STRUCT_AGENT_ID);
			//删除collect_job_classify
