...
public class StoDestStepConfActionTest extends WebBaseTestCase {
	 ...
	 * 7-4、table_id:7004,table_name:data_source,按照自定义SQL进行采集 8、table_column表测试数据： 8-1、column_id为2001-2011，模拟采集了sys_user表的前10个列，列名为user_id，create_id，dep_id，role_id，
	 ...
	 * 8-2、column_id为3001-3005，模拟采集了code_info表的所有列，列名为ci_sp_code，ci_sp_class，ci_sp_classname， ci_sp_name，ci_sp_remark
	 * 8-3、模拟自定义采集agent_info表的agent_id，agent_name，agent_type三个字段 8-4、模拟自定义采集data_source表的source_id，datasource_number，datasource_name三个字段
	 * 9、table_clean表测试数据： 9-1、对sys_user表设置一个整表字符补齐规则 9-2、对sys_user表设置一个整表字符替换规则 10、column_clean表测试数据
	 * 10-1、对sys_user表的create_id、dep_id列设置列字符补齐 10-2、对sys_user表的user_name列设置列字符替换 10-3、对sys_user表的login_date列设置了日期格式化
	 * 11、clean_parameter表测试数据 11-1、对databaseset_id为1001的数据库直连采集作业设置一个全表的字符替换和字符补齐规则 12、column_split表测试数据
	 * 12-1、对code_info表的ci_sp_name列设置字段拆分，按照偏移量，拆分为ci_s、p_name两列 12-2、对code_info表的ci_sp_classname列设置字段拆分，按照下划线拆分ci、sp、classname三列
	 ...
	 * 15、由于配置了列合并，需要把合并后的列入到table_column表中 16、构造卸数文件相关数据(data_extraction_def表) 16-1、构造采集sys_user表，仅数据抽取，存储方式为非定长，换行符回车，列分隔符|，数据字符集为UTF-8
	 ...
	 * 18、构造orig_code_info表测试数据,共有三条数据 19、构造table_storage_info表测试数据 19-1、构造数据，模拟采集agent_info表，文件格式为ORC，进数方式为增量，做历史拉链，每天存一份，存储期限为7天
	 * 19-2、构造数据，模拟采集data_source表，文件格式为TEXTFILE，进数方式为追加，不做历史拉链，不每天存一份，存储期限为1天 20、构造Dtab_relation_store表测试数据
	 * 20-1、Agent_id：是关系型数据库主键，是hbase的rowkey 20-2、agent_name：不是主键，不是cb预聚合列，不是cb的排序列，不是solr的索引列，不是hive的分区列
	 * 20-3、agent_type：不是主键，不是cb预聚合列，不是cb的排序列，不是solr的索引列，不是hive的分区列 20-4、source_id：是solr索引列
	 * 20-5、datasource_number：不是主键，不是cb预聚合列，不是cb的排序列，不是solr的索引列，不是hive的分区列 20-6、datasource_name：不是主键，不是cb预聚合列，不是cb的排序列，不是solr的索引列，不是hive的分区列
	 * 21、构造data_store_layer表测试数据 21-1、存储配置名称为SOLR，存储类型为SOLR 21-2、存储配置名称为Oralce，存储类型为Oralce
	 * 21-3、存储配置名称为ElasticSearch，存储类型为ElasticSearch 21-4、存储配置名称为HBASE，存储类型为HBASE 21-5、存储配置名称为MONGODB，存储类型为MONGODB
	 * 22、构造data_store_layer_attr表测试数据 22-1、对关系型数据库配置了database_name，database_pwd，database_drive，user_name，database_ip，database_port，jdbc_url等属性
	 * 22-2、对solr配置了SolarUrl属性 22-3、对Hbase配置了Hbase-site.xml，Core-site.xml，Hdfs-site.xml三个配置文件的路径
	 * 23、构造data_store_layer_added表测试数据 23-1、关系型数据库主键 23-2、solr索引列 23-3、hbase的rowkey 24、构造Dtab_relation_store表测试数据
	 * 24-1、agent_info表保存进入关系型数据库 24-2、agent_info表保存进入进入hbase 24-3、data_source表保存进入solr
	 *
	 * @Param: 无
	 * @return: 无
	 */
	 ...
	 * <p>
	 * 正确数据访问1：使用正确的colSetId访问，应该可以拿到4条数据，表名分别是agent_info、data_source、sys_user、code_info 错误的数据访问1：使用错误的colSetId访问，访问会失败
	 * 错误的测试用例未达到三组:
	 *
	 * @Param: 无
	 * @return: 无
	 ...
	 * <p>
	 * 正确数据访问1：使用正确的colSetId访问，应该可以拿到2条数据，表ID分别是agent_info、data_source两张表的ID 错误的数据访问1：使用错误的colSetId访问，访问会失败 错误的测试用例未达到三组:
	 * 以上所有测试用例已经可以满足要求
	 *
	 * @Param: 无
	 * @return: 无
	 ...
	 * <p>
	 * 正确数据访问1：尝试获取存储目的地为关系型数据库的详细信息 正确数据访问2：尝试获取存储目的地为solr的详细信息 正确数据访问3：尝试获取存储目的地为hbase的详细信息
	 * 错误的数据访问1：尝试获取存储目的地为ES的详细信息，因为系统里没有进行配置，所以获取不到数据 错误的测试用例未达到三组: 以上所有测试用例已经可以满足要求
	 *
	 * @Param: 无
	 * @return: 无
	 */
	 ...
	 * <p>
	 * 正确数据访问1：对sys_user表的采集仅做抽取，存储目的地为/root目录，尝试获取 错误的数据访问1：传入错误的表ID，访问失败 错误的测试用例未达到三组:
	 * getStoDestForOnlyExtract只有一个参数，以上所有测试用例已经可以满足要求
	 *
	 * @Param: 无
	 * @return: 无
	 */
	 ...
	 * <p>
	 * 正确数据访问1：模拟修改对sys_user表的存储目的地，原有的存储目的地为/root，现在把存储目的地改为/root/test 错误的数据访问1：传入错误的表ID，访问失败 错误的测试用例未达到三组:
	 * 以上所有测试用例已经可以满足要求
	 *
	 * @Param: 无
	 * @return: 无
	 ...
	 * 测试根据表ID获取该表所有的列信息
	 * <p>
	 * 正确数据访问1：agent_info表保存进入关系型数据库，所以构造获取agent_info表的三个采集列的选择列信息情况 正确数据访问2：agent_info表保存进入HBASE，所以构造获取agent_info表的三个采集列的选择列信息情况
	 * 正确数据访问3：data_source表保存进入SOLR，所以构造获取data_source表的三个采集列的选择列信息情况 正确数据访问4：agent_info没有进入SOLR，所以构造获取当用户将agent_info表保存进入SOLR时的结果集情况
	 ...
	 * 测试保存表的字段存储信息
	 * <p>
	 * 正确数据访问1：agent_info保存进入关系型数据库，现在不用agent_id做主键了，改用agent_name做主键 正确数据访问2：agent_info原来还要保存进入Hbase，现在不进入Hbase了
	 * 正确数据访问3：data_source表source_id保存进入关系型数据库做主键，保存进入hbase做rowkey，不再进solr做索引列 错误的数据访问1：保存字段特殊属性的数组为空 错误的测试用例未达到三组:
	 *
	 * @Param: 无
	 * @return: 无
	 */
	 ...
	 * <p>
	 * 正确数据访问1：更新data_source表在table_column表中的数据，并断言是否修改成功 错误的数据访问1：更新agent_info表在table_column表中的数据，但是传错一个column_id，使其故意更新不成功
	 * 错误的数据访问2：更新agent_info表在table_column表中的数据，但是故意不关联某个字段的ID 错误的场景未满三种：updateColumnZhName方法只有一个参数
	 *
	 * @Param: 无
	 * @return: 无
	 */
	 ...
	 * 测试保存表存储属性配置
	 * <p>
	 * 正确数据访问1：修改agent_info表的保存目的地，由关系型数据库和hbase修改为仅保存到关系型数据库,进数方式为替换，不进行拉链存储，只保存一天 正确数据访问2：修改data_source表的存储目的地，由SOLR变为进入MongoDB和关系型数据库
	 ...
	 * 错误的数据访问1：新增采集table_column表的存储目的地，但是没有传入任何一个目的地 错误的数据访问2：新增采集table_column表的存储目的地，但是在数据抽取定义表中没有构造table_column表的抽取信息
	 * 错误的数据访问3：新增采集table_column表的存储目的地，但是未关联表 错误的数据访问4：新增采集table_column表的存储目的地，但是未选择是否拉链存储
	 * 错误的数据访问5：新增采集table_column表的存储目的地，但是未填写存储期限 错误的数据访问6：新增采集table_column表的存储目的地，选择了拉链存储但是没有选择存储方式
	 *
	 * @Param: 无
	 * @return: 无
	 */
	...
	public void saveTbStoInfo() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			for (int i = 0; i < relationTable.getRowCount(); i++) {
				if (relationTable.getLong(i, "dsl_id") == 4400L) {
					assertThat("查询到的agent_info表在Dtab_relation_store中的数据有两条，符合期望", true, is(true));
				} else if (relationTable.getLong(i, "dsl_id") == 4402L) {
					assertThat("查询到的agent_info表在Dtab_relation_store中的数据有两条，符合期望", true, is(true));
				} else {
					assertThat("查询到的agent_info表在Dtab_relation_store表中出现了不符合期望的数据", true, is(false));
				}
			}
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			for (int i = 0; i < relationTable.getRowCount(); i++) {
				if (relationTable.getLong(i, "dsl_id") == 4400L) {
					assertThat("查询到的data_source表在Dtab_relation_store中的数据有两条，符合期望", true, is(true));
				} else if (relationTable.getLong(i, "dsl_id") == 4403L) {
					assertThat("查询到的data_source表在Dtab_relation_store中的数据有两条，符合期望", true, is(true));
				} else {
					assertThat("查询到的data_source表在Dtab_relation_store表中出现了不符合期望的数据", true, is(false));
				}
			}

			//在保存后，将因执行saveTbStoInfo<正确的数据访问2>而新增的测试数据删除掉
			int count = SqlOperator.execute(db, "delete from " + Dtab_relation_store.TableName + " where dsl_id = ?", 4403L);
		...
		//正确数据访问3：新增采集table_info表的存储目的地，目的地为关系型数据库和SOLR(注意构造数据抽取定义表中table_info表的相关数据，测试通过后，将新增进入的相关数据全部删除)
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			//由于这是模拟新增table_info表采集，保存存储目的地，所以table_storage_info表和Dtab_relation_store表肯定没有相关数据，但是需要构造table_info表在data_extraction_def中的数据
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			for (int i = 0; i < relationTable.getRowCount(); i++) {
				if (relationTable.getLong(i, "dsl_id") == 4400L) {
					assertThat("查询到的table_info表在Dtab_relation_store中的数据有两条，符合期望", true, is(true));
				} else if (relationTable.getLong(i, "dsl_id") == 4399L) {
					assertThat("查询到的table_info表在Dtab_relation_store中的数据有两条，符合期望", true, is(true));
				} else {
					assertThat("查询到的table_info表在Dtab_relation_store表中出现了不符合期望的数据", true, is(false));
				}
			}
		...
		//正确数据访问4：新增采集table_column表的存储目的地，目的地为关系型数据库和SOLR，不做拉链存储(注意构造数据抽取定义表中table_column表的相关数据，测试通过后，将新增进入的相关数据全部删除)
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			//由于这是模拟新增table_column表采集，保存存储目的地，所以table_storage_info表和Dtab_relation_store表肯定没有相关数据，但是需要构造table_column表在data_extraction_def中的数据
			Data_extraction_def tableInfoDef = new Data_extraction_def();
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			for (int i = 0; i < relationTable.getRowCount(); i++) {
				if (relationTable.getLong(i, "dsl_id") == 4400L) {
					assertThat("查询到的table_column表在Dtab_relation_store中的数据有两条，符合期望", true, is(true));
				} else if (relationTable.getLong(i, "dsl_id") == 4399L) {
					assertThat("查询到的table_column表在Dtab_relation_store中的数据有两条，符合期望", true, is(true));
				} else {
					assertThat("查询到的table_column表在Dtab_relation_store表中出现了不符合期望的数据", true, is(false));
				}
			}
	 ...
	 * <p>
	 * 正确数据访问1：测试存储目的地为<关系型数据库>的表头信息 正确数据访问2：测试存储目的地为<SOLR>的表头信息 错误的数据访问1：测试存储目的地为<MONGODB>的表头信息
	 ...
	 * <p>
	 * 正确数据访问1：测试获取存储目的地为<关系型数据库>的附加属性信息ID 正确数据访问2：测试存储目的地为<SOLR>的附加属性信息ID 错误的数据访问1：测试存储目的地为<MONGODB>的附加属性信息ID
	 * 错误的测试用例未达到三组：以上所有测试用例已经足够覆盖所有可能出现的情况了
	 *
	 * @Param: 无
	 * @return: 无
	 ...
	 * <p>
	 * 正确数据访问1：模拟保存sys_user和code_info的表名和表中文名 错误的数据访问1：在模拟保存sys_user和code_info的表名和表中文名时，传入错的sys_user表ID
	 * 错误的数据访问2：在模拟保存sys_user和code_info的表名和表中文名时，未传入sys_user表的表ID 错误的数据访问3：在模拟保存sys_user和code_info的表名和表中文名时，未传入sys_user表的表名
