...
public class CollTbConfStepActionTest extends WebBaseTestCase {
	 ...
	 * 为每个方法的单元测试初始化测试数据
	 * <p>
	 * 1、构造默认表清洗优先级 2、构造默认列清洗优先级 3、构造data_source表测试数据 4、构造agent_info表测试数据 5、构造database_set表测试数据 6、构造Collect_job_classify表测试数据
	 * 7、构建table_info测试数据 8、构建table_column表测试数据 9、构造table_storage_info表测试数据 10、构造table_clean表测试数据 11、构造column_merge表测试数据
	 * 12、插入数据
	 * <p>
	 * 测试数据： 1、默认表清洗优先级为(字符补齐，字符替换，列合并，首尾去空) 2、默认列清洗优先级为(字符补齐，字符替换，日期格式转换，码值转换，列拆分，首尾去空) 3、data_source表：有1条数据，source_id为1
	 * 4、agent_info表：有2条数据,全部是数据库采集Agent，agent_id分别为7001，7002,source_id为1 5、database_set表：有2条数据,database_id为1001,1002,
	 * agent_id分别为7001,7002，1001的classifyId是10086，1002的classifyId是10010 1001设置完成并发送成功(is_sendok)
	 * 6、collect_job_classify表：有2条数据，classify_id为10086L、10010L，agent_id分别为7001L、7002L,user_id为9997L
	 * 7、table_info表测试数据共4条，databaseset_id为1001 7-1、table_id:7001,table_name:sys_user,按照画面配置信息进行采集，并且配置了单表过滤SQL,select * from
	 * sys_user where user_id = 2001，不进行并行抽取 7-2、table_id:7002,table_name:code_info,按照画面配置信息进行采集,进行并行抽取，分页SQL为select * from
	 * code_info limit 10,设置数据量为10万，并行抽取线程数为6，每日增量数据为1000条 7-3、table_id:7003,table_name:agent_info,按照自定义SQL进行采集，不进行并行抽取
	 * 7-4、table_id:7004,table_name:data_source,按照自定义SQL进行采集，不进行并行抽取 8、table_column表测试数据：只有在画面上进行配置的采集表才会向table_column表中保存数据
	 * 8-1、column_id为2001-2010，模拟采集了sys_user表的前10个列，列名为user_id，create_id，dep_id，role_id，
	 * user_name，user_password，user_email，user_mobile，useris_admin，user_type 8-2、column_id为3001-3005，模拟采集了code_info表的所有列，列名为ci_sp_code，ci_sp_class，ci_sp_classname，
	 * ci_sp_name，ci_sp_remark 8-3、模拟自定义采集agent_info表的agent_id，agent_name，agent_type三个字段
	 * 8-4、模拟自定义采集data_source表的source_id，datasource_number，datasource_name三个字段 9、table_storage_info表测试数据：
	 * 9-1、storage_id为1234，文件格式为CSV，存储方式为替换，table_id为sys_user表的ID 9-2、storage_id为5678，文件格式为定长文件，存储方式为追加，table_id为code_info表的ID
	 * 9-3、storage_id为1273，文件格式为CSV，存储方式为替换，table_id为data_source表的ID 9-4、storage_id为4288，文件格式为定长文件，存储方式为追加，table_id为agent_info表的ID
	 * 10、table_clean表测试数据： 10-1、模拟数据对sys_user表进行整表清洗，分别做了列合并和首尾去空 10-2、模拟数据对code_info表进行了整表清洗，分别做了字符替换字符补齐，将所有列值的abc全部替换为def，将所有列值的前面补上beyond字符串
	 ...
	 * 11、column_merge表测试数据：对sys_user设置了两个列合并，对code_info表设置了一个列合并 11-1、模拟数据将sys_user表的user_id和create_id两列合并为user_create_id
	 * 11-2、模拟数据将sys_user表的user_name和user_password两列合并为user_name_password 11-3、模拟数据将code_info表的ci_sp_classname和ci_sp_name两列合并为ci_sp_classname_name
	 * 12、data_extraction_def表测试数据： 12-1、sys_user:仅抽取，需要表头，落地编码为UTF-8，数据落地格式为ORC，数据落地目录为/root
	 * 12-2、code_info:仅抽取，不需要表头，落地编码为UTF-8，数据落地格式为PARQUET，数据落地目录为/home/hyshf 12-3、data_source:抽取并入库，不需要表头，落地编码为UTF-8，数据落地格式为CSV
	 * 12-4、agent_info:抽取并入库，不需要表头，落地编码为UTF-8，数据落地格式为非定长，列分隔符为"\r"，行分隔符为"|" 13、Dtab_relation_store表测试数据：
	 * 13-1、sys_user表保存进入关系型数据库 13-2、code_info表保存进入关系型数据库 13-3、data_source表保存进入关系型数据库 13-4、agent_info表保存进入关系型数据库
	 * 14、由于该Action类的测试连接功能需要与agent端交互，所以需要配置一条agent_down_info表的记录，用于找到http访问的完整url 15、column_merge表测试数据:
...
//	  2: 使用测试任务ID之外的ID来检查是否存在采集表信息
	...
	public void getPageSQL() {
		...
		assertThat("获得的<分页抽取SQL>是select * from code_info limit 10", rightData.getString(0, "page_sql"),
			is("select * from code_info limit 10"));
	 ...
	 * <p>
	 * 正确数据访问1：构造两条自定义SQL查询设置数据，测试保存功能 正确数据访问2：模拟修改agent_info和data_source表的自定义SQL查询,则之前为agent_info和data_source表定义的脏数据都会被删除
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			for (Table_info tableInfo : expectedList) {
				if (tableInfo.getTable_name().equalsIgnoreCase("getHalfStructTask")) {
					assertThat("保存成功后，自定义SQL查询getHalfStructTask的中文名应该是<获得半结构化采集任务>", tableInfo.getTable_ch_name(),
						is("获得半结构化采集任务"));
					Result resultOne = SqlOperator.queryResult(db,
						"select is_get, is_primary_key, column_name, column_type, column_ch_name, is_alive, is_new, tc_or from "
							+ Table_column.TableName + " where table_id = ?", tableInfo.getTable_id());
					...
					for (int i = 0; i < resultOne.getRowCount(); i++) {
						if (resultOne.getString(i, "column_name").equalsIgnoreCase("odc_id")) {
							assertThat("采集列名为odc_id，is_get字段符合预期", resultOne.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为odc_id，is_primary_key字段符合预期", resultOne.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为odc_id，column_type字段符合预期",
								resultOne.getString(i, "column_type").equalsIgnoreCase("int8"), is(true));
							assertThat("采集列名为odc_id，column_ch_name字段符合预期", resultOne.getString(i, "column_ch_name"),
								is("odc_id"));
							assertThat("采集列名为odc_id，is_alive字段符合预期", resultOne.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为odc_id，is_new字段符合预期", resultOne.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为odc_id，tc_or字段符合预期", resultOne.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else if (resultOne.getString(i, "column_name").equalsIgnoreCase("object_collect_type")) {
							assertThat("采集列名为object_collect_type，is_get字段符合预期", resultOne.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为object_collect_type，is_primary_key字段符合预期",
								resultOne.getString(i, "is_primary_key"), is(IsFlag.Fou.getCode()));
							assertThat("采集列名为object_collect_type，column_type字段符合预期",
								resultOne.getString(i, "column_type").equalsIgnoreCase("bpchar(1)"), is(true));
							assertThat("采集列名为object_collect_type，column_ch_name字段符合预期",
								resultOne.getString(i, "column_ch_name"), is("object_collect_type"));
							assertThat("采集列名为object_collect_type，is_alive字段符合预期", resultOne.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为object_collect_type，is_new字段符合预期", resultOne.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为object_collect_type，tc_or字段符合预期", resultOne.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else if (resultOne.getString(i, "column_name").equalsIgnoreCase("obj_number")) {
							assertThat("采集列名为obj_number，is_get字段符合预期", resultOne.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为obj_number，is_primary_key字段符合预期", resultOne.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为obj_number，column_type字段符合预期",
								resultOne.getString(i, "column_type").equalsIgnoreCase("varchar(200)"), is(true));
							assertThat("采集列名为obj_number，column_ch_name字段符合预期", resultOne.getString(i, "column_ch_name"),
								is("obj_number"));
							assertThat("采集列名为obj_number，is_alive字段符合预期", resultOne.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为obj_number，is_new字段符合预期", resultOne.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为obj_number，tc_or字段符合预期", resultOne.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else {
					...
					Result resultTwo = SqlOperator.queryResult(db,
						"select is_get, is_primary_key, column_name, column_type, column_ch_name, is_alive, is_new, tc_or from "
							+ Table_column.TableName + " where table_id = ?", tableInfo.getTable_id());
					...
					for (int i = 0; i < resultTwo.getRowCount(); i++) {
						if (resultTwo.getString(i, "column_name").equalsIgnoreCase("ftp_id")) {
							assertThat("采集列名为ftp_id，is_get字段符合预期", resultTwo.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为ftp_id，is_primary_key字段符合预期", resultTwo.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为ftp_id，column_type字段符合预期",
								resultTwo.getString(i, "column_type").equalsIgnoreCase("int8"), is(true));
							assertThat("采集列名为ftp_id，column_ch_name字段符合预期", resultTwo.getString(i, "column_ch_name"),
								is("ftp_id"));
							assertThat("采集列名为ftp_id，is_alive字段符合预期", resultTwo.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为ftp_id，is_new字段符合预期", resultTwo.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为ftp_id，tc_or字段符合预期", resultTwo.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else if (resultTwo.getString(i, "column_name").equalsIgnoreCase("ftp_number")) {
							assertThat("采集列名为ftp_number，is_get字段符合预期", resultTwo.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为ftp_number，is_primary_key字段符合预期", resultTwo.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为ftp_number，column_type字段符合预期",
								resultTwo.getString(i, "column_type").equalsIgnoreCase("varchar(200)"), is(true));
							assertThat("采集列名为ftp_number，column_ch_name字段符合预期", resultTwo.getString(i, "column_ch_name"),
								is("ftp_number"));
							assertThat("采集列名为ftp_number，is_alive字段符合预期", resultTwo.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为ftp_number，is_new字段符合预期", resultTwo.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为ftp_number，tc_or字段符合预期", resultTwo.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else if (resultTwo.getString(i, "column_name").equalsIgnoreCase("ftp_name")) {
							assertThat("采集列名为ftp_name，is_get字段符合预期", resultTwo.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为ftp_name，is_primary_key字段符合预期", resultTwo.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为ftp_name，column_type字段符合预期",
								resultTwo.getString(i, "column_type").equalsIgnoreCase("varchar(512)"), is(true));
							assertThat("采集列名为ftp_name，column_ch_name字段符合预期", resultTwo.getString(i, "column_ch_name"),
								is("ftp_name"));
							assertThat("采集列名为ftp_name，is_alive字段符合预期", resultTwo.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为ftp_name，is_new字段符合预期", resultTwo.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为ftp_name，tc_or字段符合预期", resultTwo.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else {
			...
			int firCount = SqlOperator
				.execute(db, "delete from " + Table_info.TableName + " WHERE table_name = ?", "getHalfStructTask");
			assertThat("测试完成后，table_name为getHalfStructTask的测试数据被删除了", firCount, is(1));
			int secCount = SqlOperator
				.execute(db, "delete from " + Table_info.TableName + " WHERE table_name = ?", "getFTPTask");
			assertThat("测试完成后，table_name为getFTPTask的测试数据被删除了", secCount, is(1));

			//验证完毕后，将自己在本方法中构造的数据删除掉(table_column表)
			for (Table_info tableInfo : expectedList) {
				SqlOperator
					.execute(db, "delete from " + Table_column.TableName + " where table_id = ?", tableInfo.getTable_id());
			}
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			for (Table_info tableInfo : expectedList) {
				if (tableInfo.getTable_name().equalsIgnoreCase("agent_info_customize")) {
					assertThat("保存成功后，自定义SQL查询agent_info_customize的中文名应该是<agent信息表自定义>", tableInfo.getTable_ch_name(),
						is("agent信息表自定义"));
					Result resultOne = SqlOperator.queryResult(db,
						"select is_get, is_primary_key, column_name, column_type, column_ch_name, is_alive, is_new, tc_or from "
							+ Table_column.TableName + " where table_id = ?", tableInfo.getTable_id());
					...
					for (int i = 0; i < resultOne.getRowCount(); i++) {
						if (resultOne.getString(i, "column_name").equalsIgnoreCase("agent_ip")) {
							assertThat("采集列名为agent_ip，is_get字段符合预期", resultOne.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为agent_ip，is_primary_key字段符合预期", resultOne.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为agent_ip，column_type字段符合预期",
								resultOne.getString(i, "column_type").equalsIgnoreCase("varchar(50)"), is(true));
							assertThat("采集列名为agent_ip，column_ch_name字段符合预期", resultOne.getString(i, "column_ch_name"),
								is("agent_ip"));
							assertThat("采集列名为agent_ip，is_alive字段符合预期", resultOne.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为agent_ip，is_new字段符合预期", resultOne.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为agent_ip，tc_or字段符合预期", resultOne.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else if (resultOne.getString(i, "column_name").equalsIgnoreCase("agent_port")) {
							assertThat("采集列名为agent_port，is_get字段符合预期", resultOne.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为agent_port，is_primary_key字段符合预期", resultOne.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为agent_port，column_type字段符合预期",
								resultOne.getString(i, "column_type").equalsIgnoreCase("varchar(10)"), is(true));
							assertThat("采集列名为agent_port，column_ch_name字段符合预期", resultOne.getString(i, "column_ch_name"),
								is("agent_port"));
							assertThat("采集列名为agent_port，is_alive字段符合预期", resultOne.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为agent_port，is_new字段符合预期", resultOne.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为agent_port，tc_or字段符合预期", resultOne.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else if (resultOne.getString(i, "column_name").equalsIgnoreCase("agent_status")) {
							assertThat("采集列名为agent_status，is_get字段符合预期", resultOne.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为agent_status，is_primary_key字段符合预期", resultOne.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为agent_status，column_type字段符合预期",
								resultOne.getString(i, "column_type").equalsIgnoreCase("bpchar(1)"), is(true));
							assertThat("采集列名为agent_status，column_ch_name字段符合预期", resultOne.getString(i, "column_ch_name"),
								is("agent_status"));
							assertThat("采集列名为agent_status，is_alive字段符合预期", resultOne.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为agent_status，is_new字段符合预期", resultOne.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为agent_status，tc_or字段符合预期", resultOne.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else {
							assertThat("出现了不符合预期的情况，采集列名为：" + resultOne.getString(i, "column_name"), true, is(false));
						}
					...
					assertThat("保存成功后，自定义SQL查询data_source_customize的中文名应该是<数据源表自定义>", tableInfo.getTable_ch_name(),
						is("数据源表自定义"));
					Result resultTwo = SqlOperator.queryResult(db,
						"select is_get, is_primary_key, column_name, column_type, column_ch_name, is_alive, is_new, tc_or from "
							+ Table_column.TableName + " where table_id = ?", tableInfo.getTable_id());
					...
					for (int i = 0; i < resultTwo.getRowCount(); i++) {
						if (resultTwo.getString(i, "column_name").equalsIgnoreCase("source_remark")) {
							assertThat("采集列名为source_remark，is_get字段符合预期", resultTwo.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为source_remark，is_primary_key字段符合预期", resultTwo.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为source_remark，column_type字段符合预期",
								resultTwo.getString(i, "column_type").equalsIgnoreCase("varchar(512)"), is(true));
							assertThat("采集列名为source_remark，column_ch_name字段符合预期", resultTwo.getString(i, "column_ch_name"),
								is("source_remark"));
							assertThat("采集列名为source_remark，is_alive字段符合预期", resultTwo.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为source_remark，is_new字段符合预期", resultTwo.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为source_remark，tc_or字段符合预期", resultTwo.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else if (resultTwo.getString(i, "column_name").equalsIgnoreCase("create_date")) {
							assertThat("采集列名为create_date，is_get字段符合预期", resultTwo.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为create_date，is_primary_key字段符合预期", resultTwo.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为create_date，column_type字段符合预期",
								resultTwo.getString(i, "column_type").equalsIgnoreCase("bpchar(8)"), is(true));
							assertThat("采集列名为create_date，column_ch_name字段符合预期", resultTwo.getString(i, "column_ch_name"),
								is("create_date"));
							assertThat("采集列名为create_date，is_alive字段符合预期", resultTwo.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为create_date，is_new字段符合预期", resultTwo.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为create_date，tc_or字段符合预期", resultTwo.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else if (resultTwo.getString(i, "column_name").equalsIgnoreCase("create_time")) {
							assertThat("采集列名为create_time，is_get字段符合预期", resultTwo.getString(i, "is_get"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为create_time，is_primary_key字段符合预期", resultTwo.getString(i, "is_primary_key"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为create_time，column_type字段符合预期",
								resultTwo.getString(i, "column_type").equalsIgnoreCase("bpchar(6)"), is(true));
							assertThat("采集列名为create_time，column_ch_name字段符合预期", resultTwo.getString(i, "column_ch_name"),
								is("create_time"));
							assertThat("采集列名为create_time，is_alive字段符合预期", resultTwo.getString(i, "is_alive"),
								is(IsFlag.Shi.getCode()));
							assertThat("采集列名为create_time，is_new字段符合预期", resultTwo.getString(i, "is_new"),
								is(IsFlag.Fou.getCode()));
							assertThat("采集列名为create_time，tc_or字段符合预期", resultTwo.getString(i, "tc_or"),
								is(columnCleanOrder.toJSONString()));
						} else {
							assertThat("出现了不符合预期的情况，采集列名为：" + resultTwo.getString(i, "column_name"), true, is(false));
						}
			...
			//验证完毕后，将自己在本方法中构造的数据删除掉(table_info表)
			int firCount = SqlOperator
				.execute(db, "delete from " + Table_info.TableName + " WHERE table_name = ?", "agent_info_customize");
			assertThat("测试完成后，table_name为agent_info_customize的测试数据被删除了", firCount, is(1));
			int secCount = SqlOperator
				.execute(db, "delete from " + Table_info.TableName + " WHERE table_name = ?", "data_source_customize");
	...
	public void getColumnInfo() {
		...
		for (Map.Entry<Object, Object> entry : rightDataOne.entrySet()) {
			...
			if (key.equalsIgnoreCase("tableName")) {
				...
				assertThat("返回的结果中，key为columnInfo的Entry，value为List<Table_column>,code_info表中有5列", tableColumns.size(),
					is(5));
		...
		for (Map.Entry<Object, Object> entry : rightDataTwo.entrySet()) {
			...
			if (key.equalsIgnoreCase("tableName")) {
				...
				assertThat("返回的结果中，有一对Entry的key为columnInfo", key, is("columnInfo"));
				List<Table_column> tableColumns = (List<Table_column>) entry.getValue();
				assertThat("返回的结果中，key为columnInfo的Entry，value为List<Table_column>,ftp_collect表中有24列", tableColumns.size(),
					is(24));
			} else if (key.equalsIgnoreCase("editFlag")) {
				assertThat("返回的结果中，有一对Entry的key为editFlag", key, is("editFlag"));
				assertThat("返回的结果中，key为editFlag的Entry，value为1", entry.getValue(), is(IsFlag.Shi.getCode()));
		...
		for (Map.Entry<Object, Object> entry : wrongTableNameData.entrySet()) {
			...
			if (key.equalsIgnoreCase("tableName")) {
				...
				assertThat("返回的结果中，有一对Entry的key为columnInfo", key, is("columnInfo"));
				List<Table_column> tableColumns = (List<Table_column>) entry.getValue();
				assertThat("返回的结果中，key为columnInfo的Entry，value为List<Table_column>,没有wzc_collect这张表", tableColumns.isEmpty(),
					is(true));
			} else if (key.equalsIgnoreCase("editFlag")) {
				assertThat("返回的结果中，有一对Entry的key为editFlag", key, is("editFlag"));
				assertThat("返回的结果中，key为editFlag的Entry，value为1", entry.getValue(), is(IsFlag.Shi.getCode()));
	...
	/**
	 * 测试保存单个表的采集信息功能 正确数据访问1：在database_id为1001的数据库采集任务下构造新增采集ftp_collect表的数据，不选择采集列和列排序，设置并行抽取SQL为select * from ftp_collect
	 * limit 10;(需要和agent进行交互获取该表的字段) 正确数据访问2：在database_id为1001的数据库采集任务下构造新增采集object_collect表的数据，选择采集列和列排序,不设置并行抽取(不需要和agent进行交互)
	 * 正确数据访问3：在database_id为1001的数据库采集任务下构造修改采集code_info表的数据，选择采集列和列排序，不设置并行抽取， 注意，由于给code_info表构造了data_extraction_def表、table_clean表、table_srorage_info表、column_merge表、table_column表信息
	 * 因此对这个表的采集数据进行修改，要断言修改成功后这5张表的数据是否都被修改了。(不需要和agent进行交互) 正确数据访问4：在database_id为1001的数据库采集任务下构造新增采集ftp_collect表和object_collect表的数据，不选择采集列和列排序，不设置并行抽取(需要和agent交互)
	...
	public void saveCollTbInfoOne() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			assertThat("<正确的测试用例1>执行成功后，采集ftp_collect表配置，<清洗顺序>符合期望", afterTableInfo.getString(0, "ti_or"),
				is(tableCleanOrder.toJSONString()));
			assertThat("<正确的测试用例1>执行成功后，采集ftp_collect表配置，<是否使用MD5>符合期望", afterTableInfo.getString(0, "is_md5"),
				is(IsFlag.Shi.getCode()));
			assertThat("<正确的测试用例1>执行成功后，采集ftp_collect表配置，<是否仅登记>符合期望", afterTableInfo.getString(0, "is_register"),
				is(IsFlag.Fou.getCode()));
			...
			Result afterTableColumn = SqlOperator
				.queryResult(db, "select * from " + Table_column.TableName + " where table_id = ?",
					afterTableInfo.getLong(0, "table_id"));
			...
			for (int i = 0; i < afterTableColumn.getRowCount(); i++) {
				if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_id")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_id>字段的类型为<int8>", afterTableColumn.getString(i, "column_type"),
						is("int8"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_number")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_number>字段的类型为<varchar(200)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(200)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_name")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_name>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("start_date")) {
					assertThat("<正确的测试用例1>执行成功后, <start_date>字段的类型为<bpchar(8)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(8)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("end_date")) {
					assertThat("<正确的测试用例1>执行成功后, <end_date>字段的类型为<bpchar(8)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(8)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_ip")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_ip>字段的类型为<varchar(50)>", afterTableColumn.getString(i, "column_type"),
						is("varchar(50)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_port")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_port>字段的类型为<varchar(10)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(10)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_username")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_username>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_password")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_password>字段的类型为<varchar(100)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(100)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_dir")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_dir>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("local_path")) {
					assertThat("<正确的测试用例1>执行成功后, <local_path>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_rule_path")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_rule_path>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("child_file_path")) {
					assertThat("<正确的测试用例1>执行成功后, <child_file_path>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("child_time")) {
					assertThat("<正确的测试用例1>执行成功后, <child_time>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("file_suffix")) {
					assertThat("<正确的测试用例1>执行成功后, <file_suffix>字段的类型为<varchar(200)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(200)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_model")) {
					assertThat("<正确的测试用例1>执行成功后, <ftp_model>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("run_way")) {
					assertThat("<正确的测试用例1>执行成功后, <run_way>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("remark")) {
					assertThat("<正确的测试用例1>执行成功后, <remark>字段的类型为<varchar(512)>", afterTableColumn.getString(i, "column_type"),
						is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("is_sendok")) {
					assertThat("<正确的测试用例1>执行成功后, <is_sendok>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("is_unzip")) {
					assertThat("<正确的测试用例1>执行成功后, <is_unzip>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("reduce_type")) {
					assertThat("<正确的测试用例1>执行成功后, <reduce_type>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("is_read_realtime")) {
					assertThat("<正确的测试用例1>执行成功后, <is_read_realtime>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("realtime_interval")) {
					assertThat("<正确的测试用例1>执行成功后, <realtime_interval>字段的类型为<int8>",
						afterTableColumn.getString(i, "column_type"), is("int8"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("agent_id")) {
					assertThat("<正确的测试用例1>执行成功后, <agent_id>字段的类型为<int8>", afterTableColumn.getString(i, "column_type"),
						is("int8"));
				} else {
					assertThat("<正确的测试用例1>执行完成之后，采集ftp_collect表的所有字段，出现了不符合期望的字段，字段名为 : " + afterTableColumn
						.getString(i, "column_name"), true, is(false));
				}
			}

			//以上断言都测试成功后，删除<正确数据访问1>生成的数据
			int tableCount = SqlOperator.execute(db, "delete from " + Table_info.TableName + " where table_id = ?",
				afterTableInfo.getLong(0, "table_id"));
			int columnCount = SqlOperator.execute(db, "delete from " + Table_column.TableName + " where table_id = ?",
				afterTableInfo.getLong(0, "table_id"));
	...
	public void saveCollTbInfoTwo() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			long count = SqlOperator
				.queryNumber(db, "select count(1) from " + Table_info.TableName + " where table_name = ?", "object_collect")
				.orElseThrow(() -> new BusinessException("SQL查询错误"));
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			//新增成功后，断言数据库中的数据是否符合期望，如果符合期望，则删除本次新增带来的数据
			Result afterTableInfo = SqlOperator
				.queryResult(db, "select * from " + Table_info.TableName + " where table_name = ?", "object_collect");
			assertThat("<正确的测试用例2>执行成功后，table_info表中出现了采集object_collect表的配置", afterTableInfo.getRowCount(), is(1));
			assertThat("<正确的测试用例2>执行成功后，采集object_collect表配置，<清洗顺序>符合期望", afterTableInfo.getString(0, "ti_or"),
				is(tableCleanOrder.toJSONString()));
			assertThat("<正确的测试用例2>执行成功后，采集object_collect表配置，<是否使用MD5>符合期望", afterTableInfo.getString(0, "is_md5"),
				is(IsFlag.Shi.getCode()));
			assertThat("<正确的测试用例2>执行成功后，采集object_collect表配置，<是否仅登记>符合期望", afterTableInfo.getString(0, "is_register"),
				is(IsFlag.Fou.getCode()));
			assertThat("<正确的测试用例2>执行成功后，采集object_collect表配置，<是否并行抽取>符合期望", afterTableInfo.getString(0, "is_parallel"),
				is(IsFlag.Fou.getCode()));
			...
			for (int i = 0; i < afterTableColumn.getRowCount(); i++) {
				if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("odc_id")) {
					assertThat("<正确的测试用例2>执行成功后, <odc_id>字段的类型为<int8>", afterTableColumn.getString(i, "column_type"),
						is("int8"));
					...
					assertThat("<正确的测试用例2>执行成功后, <object_collect_type>字段的类型为<int8>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
					assertThat("<正确的测试用例2>执行成功后, <object_collect_type>字段的采集顺序为<2>",
						afterTableColumn.getString(i, "tc_remark"), is("2"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("obj_number")) {
					assertThat("<正确的测试用例2>执行成功后, <obj_number>字段的类型为<int8>", afterTableColumn.getString(i, "column_type"),
						is("varchar(200)"));
					assertThat("<正确的测试用例2>执行成功后, <obj_number>字段的采集顺序为<3>", afterTableColumn.getString(i, "tc_remark"),
						is("3"));
				} else {
					assertThat(
						"<正确的测试用例2>执行成功后, 再次查询table_column表，出现了不符合期望的情况，表名为：" + afterTableColumn.getString(i, "column_name"),
						true, is(false));
				}
			}
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			for (int i = 0; i < tableColumn.getRowCount(); i++) {
				if (tableColumn.getString(i, "column_name").equalsIgnoreCase("ci_sp_code")) {
					...
					assertThat("设置采集code_info表的所有列，但是出现了不符合期望的列，列名为 : " + tableColumn.getString(i, "column_name"), true,
						is(false));
				}
			}
			...
			assertThat("code_info表的数据抽取定义信息在Data_extraction_def表中有数据，<是否表头>符合预期",
				dataExtractionDef.getString(0, "is_header"), is(IsFlag.Fou.getCode()));
			assertThat("code_info表的数据抽取定义信息在Data_extraction_def表中有数据，<落地文件编码>符合预期",
				dataExtractionDef.getString(0, "database_code"), is(DataBaseCode.UTF_8.getCode()));
			assertThat("code_info表的数据抽取定义信息在Data_extraction_def表中有数据，<落地文件格式>符合预期",
				dataExtractionDef.getString(0, "dbfile_format"), is(FileFormat.PARQUET.getCode()));
			assertThat("code_info表的数据抽取定义信息在Data_extraction_def表中有数据，<落地存储目录>符合预期",
				dataExtractionDef.getString(0, "plane_url"), is("/home/hyshf"));
	...
	public void saveCollTbInfoFour() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			assertThat("<正确的测试用例4>执行成功后，采集ftp_collect表配置，<清洗顺序>符合期望", afterTableInfo.getString(0, "ti_or"),
				is(tableCleanOrder.toJSONString()));
			assertThat("<正确的测试用例4>执行成功后，采集ftp_collect表配置，<是否使用MD5>符合期望", afterTableInfo.getString(0, "is_md5"),
				is(IsFlag.Shi.getCode()));
			assertThat("<正确的测试用例4>执行成功后，采集ftp_collect表配置，<是否仅登记>符合期望", afterTableInfo.getString(0, "is_register"),
				is(IsFlag.Fou.getCode()));
			assertThat("<正确的测试用例4>执行成功后，采集ftp_collect表配置，<是否并行抽取>符合期望", afterTableInfo.getString(0, "is_parallel"),
				is(IsFlag.Fou.getCode()));
			...
			for (int i = 0; i < afterTableColumn.getRowCount(); i++) {
				if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_id")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_id>字段的类型为<int8>", afterTableColumn.getString(i, "column_type"),
						is("int8"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_number")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_number>字段的类型为<varchar(200)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(200)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_name")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_name>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("start_date")) {
					assertThat("<正确的测试用例4>执行成功后, <start_date>字段的类型为<bpchar(8)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(8)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("end_date")) {
					assertThat("<正确的测试用例4>执行成功后, <end_date>字段的类型为<bpchar(8)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(8)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_ip")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_ip>字段的类型为<varchar(50)>", afterTableColumn.getString(i, "column_type"),
						is("varchar(50)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_port")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_port>字段的类型为<varchar(10)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(10)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_username")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_username>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_password")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_password>字段的类型为<varchar(100)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(100)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_dir")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_dir>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("local_path")) {
					assertThat("<正确的测试用例4>执行成功后, <local_path>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_rule_path")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_rule_path>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("child_file_path")) {
					assertThat("<正确的测试用例4>执行成功后, <child_file_path>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("child_time")) {
					assertThat("<正确的测试用例4>执行成功后, <child_time>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("file_suffix")) {
					assertThat("<正确的测试用例4>执行成功后, <file_suffix>字段的类型为<varchar(200)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(200)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("ftp_model")) {
					assertThat("<正确的测试用例4>执行成功后, <ftp_model>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("run_way")) {
					assertThat("<正确的测试用例4>执行成功后, <run_way>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("remark")) {
					assertThat("<正确的测试用例4>执行成功后, <remark>字段的类型为<varchar(512)>", afterTableColumn.getString(i, "column_type"),
						is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("is_sendok")) {
					assertThat("<正确的测试用例4>执行成功后, <is_sendok>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("is_unzip")) {
					assertThat("<正确的测试用例4>执行成功后, <is_unzip>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("reduce_type")) {
					assertThat("<正确的测试用例4>执行成功后, <reduce_type>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("is_read_realtime")) {
					assertThat("<正确的测试用例4>执行成功后, <is_read_realtime>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("realtime_interval")) {
					assertThat("<正确的测试用例4>执行成功后, <realtime_interval>字段的类型为<int8>",
						afterTableColumn.getString(i, "column_type"), is("int8"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("agent_id")) {
					assertThat("<正确的测试用例4>执行成功后, <agent_id>字段的类型为<int8>", afterTableColumn.getString(i, "column_type"),
						is("int8"));
				} else {
					assertThat("<正确的测试用例4>执行完成之后，采集ftp_collect表的所有字段，出现了不符合期望的字段，字段名为 : " + afterTableColumn
						.getString(i, "column_name"), true, is(false));
				}
			}
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			assertThat("<正确的测试用例4>执行成功后，采集object_collect表配置，<清洗顺序>符合期望", afterTableInfo.getString(0, "ti_or"),
				is(tableCleanOrder.toJSONString()));
			assertThat("<正确的测试用例4>执行成功后，采集object_collect表配置，<是否使用MD5>符合期望", afterTableInfo.getString(0, "is_md5"),
				is(IsFlag.Shi.getCode()));
			assertThat("<正确的测试用例4>执行成功后，采集object_collect表配置，<是否仅登记>符合期望", afterTableInfo.getString(0, "is_register"),
				is(IsFlag.Fou.getCode()));
			assertThat("<正确的测试用例4>执行成功后，采集object_collect表配置，<是否并行抽取>符合期望", afterTableInfo.getString(0, "is_parallel"),
				is(IsFlag.Fou.getCode()));
			...
			for (int i = 0; i < afterTableColumn.getRowCount(); i++) {
				if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("odc_id")) {
					assertThat("<正确的测试用例4>执行成功后, <odc_id>字段的类型为<int8>", afterTableColumn.getString(i, "column_type"),
						is("int8"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("object_collect_type")) {
					assertThat("<正确的测试用例4>执行成功后, <object_collect_type>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("obj_number")) {
					assertThat("<正确的测试用例4>执行成功后, <obj_number>字段的类型为<varchar(200)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(200)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("obj_collect_name")) {
					assertThat("<正确的测试用例4>执行成功后, <obj_collect_name>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("system_name")) {
					assertThat("<正确的测试用例4>执行成功后, <system_name>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("host_name")) {
					assertThat("<正确的测试用例4>执行成功后, <host_name>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("local_time")) {
					assertThat("<正确的测试用例4>执行成功后, <local_time>字段的类型为<bpchar(20)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(20)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("server_date")) {
					assertThat("<正确的测试用例4>执行成功后, <server_date>字段的类型为<bpchar(20)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(20)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("s_date")) {
					assertThat("<正确的测试用例4>执行成功后, <s_date>字段的类型为<bpchar(8)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(8)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("e_date")) {
					assertThat("<正确的测试用例4>执行成功后, <e_date>字段的类型为<bpchar(8)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(8)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("database_code")) {
					assertThat("<正确的测试用例4>执行成功后, <database_code>字段的类型为<bpchar(1)>",
						afterTableColumn.getString(i, "column_type"), is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("run_way")) {
					assertThat("<正确的测试用例4>执行成功后, <run_way>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("file_path")) {
					assertThat("<正确的测试用例4>执行成功后, <file_path>字段的类型为<varchar(512)>",
						afterTableColumn.getString(i, "column_type"), is("varchar(512)"));
				} else if (afterTableColumn.getString(i, "column_name").equalsIgnoreCase("is_sendok")) {
					assertThat("<正确的测试用例4>执行成功后, <is_sendok>字段的类型为<bpchar(1)>", afterTableColumn.getString(i, "column_type"),
						is("bpchar(1)"));
					...
					assertThat(
						"<正确的测试用例4>执行成功后, 再次查询table_column表，出现了不符合期望的情况，表名为：" + afterTableColumn.getString(i, "column_name"),
						true, is(false));
				}
			}
	...
	public void saveCollTbInfoFive() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			long countOne = SqlOperator
				.queryNumber(db, "select count(1) from " + Table_info.TableName + " where table_id = ?", SYS_USER_TABLE_ID)
				.orElseThrow(() -> new BusinessException("SQL查询错误"));
			...
			long countThree = SqlOperator
				.queryNumber(db, "select count(1) from " + Table_column.TableName + " where table_id = ?", SYS_USER_TABLE_ID)
				.orElseThrow(() -> new BusinessException("SQL查询错误"));
			...
			long countSeven = SqlOperator
				.queryNumber(db, "select count(1) from " + Table_clean.TableName + " where table_id = ?", SYS_USER_TABLE_ID)
				.orElseThrow(() -> new BusinessException("SQL查询错误"));
			...
			long countNine = SqlOperator
				.queryNumber(db, "select count(1) from " + Column_merge.TableName + " where table_id = ?", SYS_USER_TABLE_ID)
				.orElseThrow(() -> new BusinessException("SQL查询错误"));
	...
	public void getSQLInfoByColSetId() {
		...
		for (int i = 0; i < rightData.getRowCount(); i++) {
			if (rightData.getLong(i, "table_id") == SYS_USER_TABLE_ID) {
				...
				assertThat("code_info表设置了并行抽取，并且分页抽取SQL为<select * from code_info limit 10>",
					rightData.getString(i, "is_parallel"), is(IsFlag.Shi.getCode()));
				assertThat("code_info表设置了并行抽取，并且分页抽取SQL为<select * from code_info limit 10>",
					rightData.getString(i, "page_sql"), is("select * from code_info limit 10"));
			...
			if (key.equalsIgnoreCase("sys_user")) {
				...
				for (int i = 0; i < result.getRowCount(); i++) {
					if (result.getString(i, "column_name").equalsIgnoreCase("user_id")) {
						...
						assertThat("sys_user获取到了不符合期望的采集列,column_name为：" + result.getString(i, "column_name"), true,
							is(false));
					}
				}
				...
				for (int i = 0; i < result.getRowCount(); i++) {
					if (result.getString(i, "column_name").equalsIgnoreCase("ci_sp_code")) {
						...
						assertThat("<ci_sp_class>列中文名为<ci_sp_class>", result.getString(i, "column_ch_name"),
							is("ci_sp_class"));
					} else if (result.getString(i, "column_name").equalsIgnoreCase("ci_sp_classname")) {
						assertThat("<ci_sp_classname>列中文名为<ci_sp_classname>", result.getString(i, "column_ch_name"),
							is("ci_sp_classname"));
						...
						assertThat("<ci_sp_remark>列中文名为<ci_sp_remark>", result.getString(i, "column_ch_name"),
							is("ci_sp_remark"));
					} else {
