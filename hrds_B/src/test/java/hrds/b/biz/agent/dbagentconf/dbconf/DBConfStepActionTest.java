...
public class DBConfStepActionTest extends WebBaseTestCase {
	...
	private final InitAndDestDataForDBConf initData = new InitAndDestDataForDBConf();
	//测试数据用户ID
	 ...
	 * 为每个方法的单元测试初始化测试数据
	 * <p>
	 * 1、构建data_source表测试数据 2、构建agent_info表测试数据 3、构建database_set表测试数据，只构建数据库直连采集 4、构建collect_job_classify表测试数据 5、插入数据
	 * <p>
	 * 测试数据： 1、data_source表：有1条数据，source_id为1 2、Agent_info表：有2条数据,全部是数据库采集Agent，agent_id分别为7001，7002,source_id为1
	 * 3、database_set表：有2条数据,database_id为1001,1002, agent_id分别为7001,7002，1001的classifyId是10086，1002的classifyId是10010
	 * 1001设置完成并发送成功(is_sendok) 4、collect_job_classify表：有3条数据，classify_id为10086L、10010L、12306L，agent_id分别为7001L、7002L，7001L,user_id为9997L
	 */
	@Before
	public void before() {
		initData.before();
	 ...
	 * <p>
	 * 正确数据访问1：传入is_sendok字段为1的databaseId 错误的数据访问1：传入is_sendok字段为0的databaseId 错误的数据访问2：传入不存在的databaseId
