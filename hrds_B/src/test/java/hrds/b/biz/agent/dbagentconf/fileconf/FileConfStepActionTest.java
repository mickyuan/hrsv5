...
public class FileConfStepActionTest extends WebBaseTestCase {
	...
	private final InitAndDestDataForFileConf initData = new InitAndDestDataForFileConf();
	private final long FIRST_DATABASE_SET_ID = initData.baseInitData.FIRST_DATABASE_SET_ID;
	private final long SYS_USER_TABLE_ID = initData.baseInitData.SYS_USER_TABLE_ID;
	private final long CODE_INFO_TABLE_ID = initData.baseInitData.CODE_INFO_TABLE_ID;
	private final long AGENT_INFO_TABLE_ID = initData.baseInitData.AGENT_INFO_TABLE_ID;

	/**
	 * 为每个方法的单元测试初始化测试数据
	 * <p>
	 * 1、构造默认表清洗优先级 2、构造默认列清洗优先级 3、构造data_source表测试数据 4、构造agent_info表测试数据 5、构造database_set表测试数据 6、构造Collect_job_classify表测试数据
	 * 7、构建table_info测试数据 8、构建table_column表测试数据
	 * <p>
	 * 12、插入数据
	 * <p>
	 * 测试数据： 1、默认表清洗优先级为(字符补齐，字符替换，列合并，首尾去空) *      2、默认列清洗优先级为(字符补齐，字符替换，日期格式转换，码值转换，列拆分，首尾去空) *
	 * 3、data_source表：有1条数据，source_id为1 *      4、agent_info表：有2条数据,全部是数据库采集Agent，agent_id分别为7001，7002,source_id为1 *
	 * 5、database_set表：有2条数据,database_id为1001,1002, agent_id分别为7001,7002，1001的classifyId是10086，1002的classifyId是10010 *
	 * 1001设置完成并发送成功(is_sendok) *      6、collect_job_classify表：有3条数据，classify_id为10086L、10010L，12306L，agent_id分别为7001L、7002L,7001L，user_id为9997L
	 * *      7、table_info表测试数据共4条，databaseset_id为1001 *          7-1、table_id:7001,table_name:sys_user,按照画面配置信息进行采集 *
	 * 7-2、table_id:7002,table_name:code_info,按照画面配置信息进行采集 *          7-3、table_id:7003,table_name:agent_info,按照自定义SQL进行采集 *
	 *         7-4、table_id:7004,table_name:data_source,按照自定义SQL进行采集 *      8、table_column表测试数据：只有在画面上进行配置的采集表才会向table_column表中保存数据
	 ...
	 * ci_sp_name，ci_sp_remark *      9、table_clean表测试数据： *          9-1、对sys_user表设置一个整表字符补齐规则 * 9-2、对sys_user表设置一个整表字符替换规则
	 * *      10、column_clean表测试数据 10-1、对sys_user表的create_id、dep_id列设置列字符补齐 10-2、对sys_user表的user_name列设置列字符替换
	 * 10-3、对sys_user表的login_date列设置了日期格式化 11、clean_parameter表测试数据 11-1、对databaseset_id为1001的数据库直连采集作业设置一个全表的字符替换和字符补齐规则
	 * 12、column_split表测试数据 12-1、对code_info表的ci_sp_name列设置字段拆分，按照偏移量，拆分为ci_s、p_name两列 12-2、对code_info表的ci_sp_classname列设置字段拆分，按照下划线拆分ci、sp、classname三列
	 * 13、由于配置了列拆分，需要把拆分后的列加入到table_column表中 14、column_merge表测试数据 14-1、对sys_user表中的user_mobile和useris_admin合并成列，名叫user_mobile_admin
	 * 15、由于配置了列合并，需要把合并后的列入到table_column表中 16、构造卸数文件相关数据 16-1、构造采集sys_user表，仅数据抽取，存储方式为非定长，换行符回车，列分隔符|，数据字符集为UTF-8
	 * 16-2、构造采集code_info表，仅数据抽取，存储方式为定长，数据字符集为UTF-8 16-3、构造采集agent_info表，抽取并入库，存储方式为ORC，数据字符集为UTF-8
	 * 16-4、构造采集data_source表，抽取并入库，非定长，换行符为|，列分隔符为空格，数据字符集为UTF-8 17、构造orig_syso_info表测试数据，里面是当前系统中所有的码值信息,共有三条数据
	 ...
	 * <p>
	 * 正确数据访问1：使用正确的colSetId访问，应该可以拿到4条数据 错误的数据访问1：使用错误的colSetId访问，应该拿不到任何数据，但是不会报错，访问正常返回 错误的测试用例未达到三组:getInitInfo只有一个参数
	 */
	@Test
	public void getInitInfo() {
		...
		for (int i = 0; i < rightResultOne.getRowCount(); i++) {
			JSONArray tableDataArr = (JSONArray) rightResultOne.getObject(i, "tableData");
			for (int j = 0; j < tableDataArr.size(); j++) {
				JSONObject tableData = tableDataArr.getJSONObject(j);
				if (tableData.get("table_name").equals("sys_user")) {
					assertThat("获取到的非自定义采集表为sys_user，得到的表中文名为<用户表>", tableData.get("table_ch_name"), is("用户表"));
					assertThat("获取到的非自定义采集表为sys_user，得到数据抽取方式为<仅数据抽取>", tableData.get("data_extract_type"),
						is(DataExtractType.ShuJuKuChouQuLuoDi.getCode()));
					assertThat("获取到的非自定义采集表为sys_user，得到的数据存储格式为<非定长>", tableData.get("dbfile_format"),
						is(FileFormat.FeiDingChang.getCode()));
					assertThat("获取到的非自定义采集表为sys_user，得到的行分隔符为<空格符>", tableData.get("row_separator"), is("\n"));
					assertThat("获取到的非自定义采集表为sys_user，得到的列分隔符为<|>", tableData.get("database_separatorr"), is("|"));
					assertThat("获取到的非自定义采集表为sys_user，得到的数据字符集为<UTF-8>", tableData.get("database_code"),
						is(DataBaseCode.UTF_8.getCode()));
				} else if (tableData.get("table_name").equals("code_info")) {
					assertThat("获取到的非自定义采集表为code_info，得到的表中文名为<代码信息表>", tableData.get("table_ch_name"), is("代码信息表"));
					assertThat("获取到的非自定义采集表为code_info，得到数据抽取方式为<仅数据抽取>", tableData.get("data_extract_type"),
						is(DataExtractType.ShuJuKuChouQuLuoDi.getCode()));
					assertThat("获取到的非自定义采集表为code_info，得到的数据存储格式为<定长>", tableData.get("dbfile_format"),
						is(FileFormat.DingChang.getCode()));
					assertThat("获取到的非自定义采集表为code_info，没有指定行分隔符", tableData.get("row_separator"), is(""));
					assertThat("获取到的非自定义采集表为code_info，没有指定列分隔符", tableData.get("database_separatorr"), is(""));
					assertThat("获取到的非自定义采集表为code_info，得到的数据字符集为<UTF-8>", tableData.get("database_code"),
						is(DataBaseCode.UTF_8.getCode()));
				} else if (tableData.get("table_name").equals("agent_info")) {
					assertThat("获取到的非自定义采集表为agent_info，得到的表中文名为<Agent信息表>", tableData.get("table_ch_name"), is("Agent信息表"));
					assertThat("获取到的非自定义采集表为agent_info，得到数据抽取方式为<数据抽取并入库>", tableData.get("data_extract_type"),
						is(DataExtractType.ShuJuKuChouQuLuoDi.getCode()));
					assertThat("获取到的非自定义采集表为agent_info，得到的数据存储格式为<非定长>", tableData.get("dbfile_format"),
						is(FileFormat.ORC.getCode()));
					assertThat("获取到的非自定义采集表为agent_info，不能指定行分隔符", tableData.get("row_separator"), is(""));
					assertThat("获取到的非自定义采集表为agent_info，不能指定列分隔符", tableData.get("database_separatorr"), is(""));
					assertThat("获取到的非自定义采集表为agent_info，得到的数据字符集为<UTF-8>", tableData.get("database_code"),
						is(DataBaseCode.UTF_8.getCode()));
				} else if (tableData.get("table_name").equals("data_source")) {
					assertThat("获取到的非自定义采集表为data_source，得到的表中文名为<用户表>", tableData.get("table_ch_name"), is("数据源表"));
					assertThat("获取到的非自定义采集表为data_source，得到数据抽取方式为<仅数据抽取>", tableData.get("data_extract_type"),
						is(DataExtractType.ShuJuKuChouQuLuoDi.getCode()));
					assertThat("获取到的非自定义采集表为data_source，得到的数据存储格式为<非定长>", tableData.get("dbfile_format"),
						is(FileFormat.FeiDingChang.getCode()));
					assertThat("获取到的非自定义采集表为data_source，得到的行分隔符为<空格符>", tableData.get("row_separator"), is("|"));
					assertThat("获取到的非自定义采集表为data_source，得到的列分隔符为<|>", tableData.get("database_separatorr"), is(" "));
					assertThat("获取到的非自定义采集表为data_source，得到的数据字符集为<UTF-8>", tableData.get("database_code"),
						is(DataBaseCode.UTF_8.getCode()));
				} else {
					assertThat("获取到了不符合期望的数据，表名为" + tableData.get("table_name"), true, is(false));
				}
			}
	 ...
	 * 测试保存定义卸数文件配置
	 * <p>
	 * 正确数据访问1：对sys_user修改保存，将存储目的地从/root，修改为/home/wzc 正确数据访问2：对code_info和agent_info保存修改，code_info定义为抽取并入库，保存为PARQUET格式，字符集为UTF-8；agent_info定义为仅数据抽取，抽取成CSV格式，不指定行分隔符合列分隔符，字符集为UTF-8，保存目录为/root
	 * 错误的数据访问1：对sys_user修改保存，将存储目的地从/root，修改为/home/wzc,但是保存为非定长文件，没有指定行分隔符 错误的数据访问2：对sys_user修改保存，将存储目的地从/root，修改为/home/wzc,但是保存为非定长文件，没有指定列分隔符
	 * 错误的数据访问3：将code_info定义为抽取并入库，保存为PARQUET格式，但是指定了行分隔符 错误的数据访问4：将code_info定义为抽取并入库，保存为PARQUET格式，但是指定了列分隔符
	 * 错误的数据访问5：将code_info定义为抽取并入库，保存为ORC格式，但是指定了行分隔符 错误的数据访问6：将code_info定义为抽取并入库，保存为ORC格式，但是指定了列分隔符
	 * 错误的数据访问7：将code_info定义为抽取并入库，保存为SEQUENCEFILE格式，但是指定了行分隔符 错误的数据访问8：将code_info定义为抽取并入库，保存为SEQUENCEFILE格式，但是指定了列分隔符
	 * 错误的数据访问9：将code_info定义为抽取并入库，保存为非定长格式，但是没有指定行分隔符 错误的数据访问10：将code_info定义为抽取并入库，保存为非定长格式，但是没有指定列分隔符
	 * 错误的数据访问11：将code_info定义为抽取并入库，保存为定长格式 错误的数据访问12：将code_info定义为仅抽取，保存为ORC格式 错误的数据访问13：将code_info定义为仅抽取，保存为Parquet格式
	 * 错误的数据访问14：将code_info定义为仅抽取，保存为SEQUENCEFILE格式 错误的测试用例未达到三组:
	 */
	@Test
	public void saveFileConf() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			Result result = SqlOperator
				.queryResult(db, "select * from " + Data_extraction_def.TableName + " where table_id = ?",
					SYS_USER_TABLE_ID);
			assertThat("查询得到的数据有一条", result.getRowCount(), is(1));
			assertThat("sys_user表配置的数据抽取定义中，抽取方式为<仅数据抽取>", result.getString(0, "data_extract_type"),
				is(DataExtractType.ShuJuKuChouQuLuoDi.getCode()));
			assertThat("sys_user表配置的数据抽取定义中，卸数文件格式为<非定长>", result.getString(0, "dbfile_format"),
				is(FileFormat.FeiDingChang.getCode()));
			assertThat("sys_user表配置的数据抽取定义中，行分隔符为<回车符>", result.getString(0, "row_separator"),
				is(StringUtil.string2Unicode("\\n")));
			assertThat("sys_user表配置的数据抽取定义中，列分隔符为<|>", result.getString(0, "database_separatorr"),
				is(StringUtil.string2Unicode("|")));
			assertThat("sys_user表配置的数据抽取定义中，数据落地目录为</home/wzc>",
				result.getString(0, "plane_url").equalsIgnoreCase("/home/wzc"), is(true));
			assertThat("sys_user表配置的数据抽取定义中，数据落地编码为<UTF-8>", result.getString(0, "database_code"),
				is(DataBaseCode.UTF_8.getCode()));
		}
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			Result result = SqlOperator
				.queryResult(db, "select * from " + Data_extraction_def.TableName + " where table_id = ?",
					CODE_INFO_TABLE_ID);
			assertThat("查询得到的数据有一条", result.getRowCount(), is(1));
			assertThat("code_info表配置的数据抽取定义中，抽取方式为<数据抽取并入库>", result.getString(0, "data_extract_type"),
				is(DataExtractType.ShuJuKuChouQuLuoDi.getCode()));
			assertThat("code_info表配置的数据抽取定义中，卸数文件格式为<PARQUET>", result.getString(0, "dbfile_format"),
				is(FileFormat.PARQUET.getCode()));
			...
			assertThat("code_info表配置的数据抽取定义中，PARQUET文件不能定义数据落地目录", result.getString(0, "plane_url"), is("/root/home"));
			assertThat("code_info表配置的数据抽取定义中，数据落地编码为<UTF-8>", result.getString(0, "database_code"),
				is(DataBaseCode.UTF_8.getCode()));

			Result resultTwo = SqlOperator
				.queryResult(db, "select * from " + Data_extraction_def.TableName + " where table_id = ?",
					AGENT_INFO_TABLE_ID);
			assertThat("查询得到的数据有一条", resultTwo.getRowCount(), is(1));
			assertThat("agent_info表配置的数据抽取定义中，抽取方式为<仅数据抽取>", resultTwo.getString(0, "data_extract_type"),
				is(DataExtractType.ShuJuKuChouQuLuoDi.getCode()));
			assertThat("agent_info表配置的数据抽取定义中，卸数文件格式为<CSV>", resultTwo.getString(0, "dbfile_format"),
				is(FileFormat.CSV.getCode()));
			...
			assertThat("agent_info表配置的数据抽取定义中，定义数据落地目录为</root>",
				resultTwo.getString(0, "plane_url").equalsIgnoreCase("/root"), is(true));
			assertThat("agent_info表配置的数据抽取定义中，数据落地编码为<UTF-8>", resultTwo.getString(0, "database_code"),
				is(DataBaseCode.UTF_8.getCode()));
		}
