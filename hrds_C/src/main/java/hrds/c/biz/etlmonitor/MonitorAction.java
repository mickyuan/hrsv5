...
import hrds.commons.entity.Etl_job_disp_his;
...
public class MonitorAction extends BaseAction {
  ...
  public Map<String, Object> monitorCurrentBatchInfo(String etl_sys_cd) {
    ...
    asmSql.clean();
    asmSql.addSql(
        "SELECT T1.sub_sys_cd,T1.etl_sys_cd,(CASE WHEN T1.sub_sys_cd = T2.sub_sys_cd THEN"
            ...
            + " T2 ON T1.sub_sys_cd = T2.sub_sys_cd "
    ...
    asmSql.addSql(
        " and A.curr_bath_date=B.curr_bath_date group by A.curr_bath_date,A.etl_sys_cd,"
  ...
  @Method(
      ...
      logicStep =
          "1.数据可访问权限处理方式，通过user_id进行权限控制" + "2.判断工程是否存在" + "3.判断工程对应任务是否存在" + "4.查询当前系统运行任务下的作业信息")
  @Param(name = "etl_sys_cd", desc = "工程编号", range = "新增工程时生成")
  ...
  public List<Map<String, Object>> searchMonitorJobStateBySubCd(
    ...
    return Dbo.queryList(
        "select etl_job,curr_st_time,curr_st_time,curr_end_time,job_disp_status,"
            + "sub_sys_cd,curr_bath_date,etl_sys_cd FROM "
            + Etl_job_cur.TableName
  ...
  public List<Map<String, Object>> monitorHistoryBatchInfo(
    ...
    // 4.查询历史批量信息
    return Dbo.queryList(
        "SELECT MAX(his.curr_end_time) AS curr_end_time,"
  ...
  @Method(
      desc = "监控当前历史批量任务下的作业信息",
      logicStep = "1.数据可访问权限处理方式，通过user_id进行权限控制" + "2.判断工程是否存在" + "3.判断工程下任务是否存在" + "4.查询历史批量作业信息")
  @Param(name = "etl_sys_cd", desc = "工程编号", range = "新增工程时生成")
  @Param(name = "sub_sys_cd", desc = "任务编号", range = "新增任务时生成")
  ...
  public List<Map<String, Object>> searchMonitorHisBatchJobBySubCd(
    ...
    // 4.查询历史批量信息
    return Dbo.queryList(
        "SELECT t1.etl_sys_cd,t1.sub_sys_cd,t1.etl_job,"
  ...
  public Map<String, Object> monitorCurrJobInfo(String etl_sys_cd, String etl_job) {
    ...
    // 4.判断作业名称是否为空，不为空，查询当前工程下作业信息
    if (StringUtil.isNotBlank(etl_job)) {
      Map<String, Object> etlJobCur =
          Dbo.queryOneObject(
              "select * from "
                  + Etl_job_cur.TableName
                  + " where etl_sys_cd=? and etl_job=? order by curr_bath_date desc",
              etl_sys_cd,
              etl_job);
      // 4.1查询当前工程下作业资源分配信息
      Map<String, Object> resourceRelation =
          Dbo.queryOneObject(
              "select resource_type,resource_req from "
  ...
  public Map<String, Object> monitorHistoryJobInfo(
      String etl_sys_cd,
      String etl_job,
      String start_date,
      String end_date,
      String isHistoryBatch) {
    // 1.数据可访问权限处理方式，通过user_id进行权限控制
    // 2.判断工程是否存在
    if (!ETLJobUtil.isEtlSysExist(etl_sys_cd, getUserId())) {
      throw new BusinessException("当前工程已不存在！");
    ...
    if (StringUtil.isNotBlank(etl_job)) {
      ...
      asmSql.clean();
      asmSql.addSql(
          "SELECT etl_job,curr_bath_date,curr_st_time,curr_end_time,(CASE WHEN job_disp_status=?"
              + " THEN  '"
              + Job_Status.DONE.getValue()
              + "("
      ...
      asmSql.addSql(
          ") A INNER JOIN (SELECT curr_bath_date,etl_sys_cd,etl_job,sub_sys_cd,"
              + " MAX(curr_st_time) AS curr_st_time FROM "
              + Etl_job_disp_his.TableName
              + " WHERE etl_sys_cd=? and etl_job=?");
      asmSql.addParam(etl_sys_cd);
      asmSql.addParam(etl_job);
  ...
  private Result downEtlJobDependencyInfo(String pre_etl_sys_cd, String etl_job) {
    ...
    Result downJob =
        Dbo.queryResult(
            "select ejd.etl_sys_cd,ejd.etl_job,ed.pre_etl_job from "
  ...
  private Result topEtlJobDependencyInfo(String etl_job, String etl_sys_cd) {
    ...
    Result topJob =
        Dbo.queryResult(
            "select ejd.etl_sys_cd,ed.pre_etl_job,ejd.etl_job FROM "
                + Etl_dependency.TableName
                + " ed join "
                + Etl_job_def.TableName
                + " ejd on ed.etl_job=ejd.etl_job and ed.etl_sys_cd=ejd.etl_sys_cd "
                + " where ed.etl_job=? and ed.etl_sys_cd=?",
            etl_job,
            etl_sys_cd);
    // 3.如果上游作业依赖关系不为空，设置字节点
    if (!topJob.isEmpty()) {
  ...
  private void topNodeConnectToNode(
      Map<String, Node> nodeMap, List<String> list_Edge, String etl_job, String pre_etl_job) {
    // 1.数据可访问权限处理方式，该方法不需要权限控制
    // 2.判断当前作业是否与上游作业相同，相同跳过不相同建立依赖关系
  ...
  public Map<String, Object> monitorSystemResourceInfo(String etl_sys_cd) {
    ...
    // 3.获取资源信息
    List<Map<String, Object>> etlResourceList =
        Dbo.queryList(
            ...
            etl_sys_cd);
    // 4.获取当前工程下运行的作业
    List<Map<String, Object>> jobRunList =
        Dbo.queryList(
  ...
  public String readHistoryJobLogInfo(String etl_sys_cd, String etl_job, Integer readNum) {
    ...
    String logDir =
        etl_job_def.getLog_dic()
            + etl_job
            + "_"
            + etl_sys.getCurr_bath_date().replaceAll("-", "")
            + ".log";
    // 7.判断查询行数是否超过1000行，最大显示1000行
    if (readNum > 1000) {
      readNum = 1000;
    ...
    return ReadLog.readAgentLog(
        logDir,
        etl_sys.getEtl_serv_ip(),
        etl_sys.getEtl_serv_port(),
        etl_sys.getUser_name(),
        etl_sys.getUser_pwd(),
        readNum);
  }
  ...
  @Method(desc = "监控所有项目图表数据", logicStep = "数据可访问权限处理方式，通过user_id进行权限控制" + "2.获取当前用户下所有工程调度监控运行状态")
