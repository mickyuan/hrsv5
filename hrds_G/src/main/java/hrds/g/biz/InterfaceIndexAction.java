...
public class InterfaceIndexAction extends BaseAction {
	...
	public Result InterfaceResponseTime() {
		...
		return Dbo.queryResult("select avg(response_time) avg,max(response_time) max," +
				"min(response_time) min,interface_name,interface_user_id from "
				+ Interface_use_log.TableName + " t1 left join " + Sys_user.TableName +
				" where t1.request_state=? and t2.create_id=? " +
				" group by t1.interface_name,t1.interface_user_id", "NORMAL", getUserId());
	}
