...
// 创建默认config对象
		...
		// 设置公开的ip地址
		netConfig.setPublicAddress(ControlConfigure.HazelcastConfig.localAddress);
		//设置端口的起始地址
		netConfig.setPort(ControlConfigure.HazelcastConfig.autoIncrementPort);
		//端口被占用会+1查看是否可用，如果还是不能用会继续向后探查直到autoIncrementPort+portCount
		netConfig.setPortAutoIncrement(true);
		netConfig.setPortCount(ControlConfigure.HazelcastConfig.portCount);
		// 获取join元素<join></join>
		...
		tcpIpConfig.addMember(ControlConfigure.HazelcastConfig.localAddress);
