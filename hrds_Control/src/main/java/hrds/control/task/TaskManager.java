...
//-----------------------------分析并加载需要立即启动的作业信息用（loadReadyJob方法）start--------------------------------
	 ...
	 *
	 * @param strSystemCode 含义：调度系统编号。
	 *                      取值范围：不能为null。
	 * @param strBathDate   含义：跑批日期。
	 ...
	 * 3.时间依赖时，计算出执行时间。
	 *
	 * @param strCurrBathDate 含义：当前调度日期。
	 *                        取值范围：yyyyMMdd格式字符串，不能为null。
	 * @author Tiger.Wang
	 * @date 2019/10/8
	 */
	 ...
	 *
	 * @param exeJob 含义：表示一个待调度（执行）的作业。
	 *               取值范围：不能为null。
	 * @return boolean
	 * 含义：该作业是否需要马上执行。
	 * 取值范围：true/false。
	 ...
	 *
	 * @param frequancy   含义：作业调度频率类型。
	 *                    取值范围：Dispatch_Frequency枚举值，不能为null。
	 * @param nDispOffset 含义：调度偏移量。
	 *                    取值范围：任意数值。
	 * @param currDateStr 含义：当前跑批日期。
	 *                    取值范围：yyyyMMdd格式字符串，不能为null。
	 * @param exe_num     含义：执行次数。
	 *                    取值范围：任意数值。
	 * @param com_exe_num 含义：已经执行次数。
	 *                    取值范围：任意数值。
	 * @param star_time   含义：开始执行时间。
	 *                    取值范围：yyyyMMdd HHmmss格式字符串，不能为null。
	 * @param end_time    含义：结束执行时间。
	 *                    取值范围：yyyyMMdd HHmmss格式字符串，不能为null。
	 * @return boolean
	 * 含义：是否要调度该作业。
	 * 取值范围：true/false。
	 ...
	 * 2.检查作业状态是否为Done。
	 *
	 * @param currBathDate 含义：调度日期。
	 *                     取值范围：yyyyMMdd格式字符串，不能为null。
	 * @param jobName      含义：作业名（作业标识）。
	 ...
	 *
	 * @param waitJobInfo 含义：表示一个作业类型为WF的作业。
	 *                    取值范围：不能为null。
	 * @author Tiger.Wang
	 * @date 2019/10/9
	 */
	 ...
	 *
	 * @param etlJobCur 含义：表示一个已登记执行的作业。
	 *                  取值范围：不能为null。
