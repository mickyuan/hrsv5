...
public class DepartmentActionTest extends WebBaseTestCase {
	...
	public void before() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			ActionResult ar = JsonUtil.toObjectSafety(bodyString, ActionResult.class)
					.orElseThrow(() -> new BusinessException("模拟登录失败"));
	...
	@Method(desc = "新增部门测试方法", logicStep = "1.正确数据访问" +
			"1-1.新增部门")
	@Test
	public void addDepartmentInfo() {
		...
		ActionResult ar = JsonUtil.toObjectSafety(bodyString, ActionResult.class)
				.orElseThrow(() -> new RuntimeException("获取ActionResult失败!"));
		assertThat(ar.isSuccess(), is(true));
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			//1-1-1.检查 Department_info 部门是否新增成功
			long number = SqlOperator.queryNumber(db, "select count(*) from " + Department_info.TableName +
					" where dep_name=?", "测试添加部门init-hll")
					.orElseThrow(() -> new RuntimeException("统计测试数据结果的SQL执行失败!"));
	...
	@Method(desc = "删除部门信息测试类", logicStep = "1.正确数据访问" +
			"1-1.等待删除的数据源id存在" +
			"2.错误数据访问" +
			"2-1.待删除的数据源id不存在")
	@Test
	...
	@Method(desc = "修改部门信息测试类", logicStep = "1.正确数据访问" +
			"1-1.部门id存在" +
			"2.错误数据访问" +
			"2-1.部门id不存在")
	@Test
	public void updateDepartmentInfo() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			long number = SqlOperator.queryNumber(db, "select count(*) from " + Department_info.TableName +
					...
					.orElseThrow(() -> new RuntimeException("统计测试数据结果的SQL执行失败!"));
			assertThat("检查表 DepartmentInfo 数据修改成功", number, is(1L));
			//1-1-2.检查数据更新是否正确
	...
	@Method(desc = "获取所有部门信息测试方法", logicStep = "无参数校验结果集")
	@Test
