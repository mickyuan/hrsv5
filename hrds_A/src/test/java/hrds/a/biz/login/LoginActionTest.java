...
public class LoginActionTest extends WebBaseTestCase {
	...
	public void before() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			//	    2 : 插入部门信息数据
			int depNum = SqlOperator.execute(
							db,
							"insert into "
	...
	public void login() {
		...
		// 1 : 模拟用户名为空的情况
		String userEmptyMsg = new HttpClient()
						.addData("username", "")
		...
		//		//2 : 模拟密码为空的情况
		String pwdErrorMsg = new HttpClient()
						.addData("username", "-1000")
						.addData("password", "")
						.post(getActionUrl("login"))
						.getBodyString();
		...
		//		* <p>3 : 模拟用户名错误的情况</p>
		String userErrorMsg = new HttpClient()
						.addData("username", "-1")
		...
		//	    * <p>4 : 模拟密码错误的情况</p>
		String userPwdErrorMsg = new HttpClient()
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			String deptErrorMsg = new HttpClient()
							.addData("username", testUser)
			...
			//		5-2 : 再将部门信息还原
			int addDdepNum = SqlOperator.execute(
							db,
							"insert into "
											+ Department_info.TableName
											+ "(dep_id, dep_name,create_date,create_time) values"
			...
			// 5-3 : 发送正确的数据请求
			String successRequestMsg = new HttpClient()
							.addData("username", testUser)
							.addData("password", testUserPwd)
							.post(getActionUrl("login"))
							.getBodyString();
		...
		String responseValue = new HttpClient()
						...
						.post(getUrlActionPattern() + "/" + AppinfoConf.AppBasePackage.replace(".", "/")
										+ "/a/biz/zauth/loginCheck").getBodyString();
