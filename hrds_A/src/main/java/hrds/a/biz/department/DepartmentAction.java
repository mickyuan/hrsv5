...
public class DepartmentAction extends BaseAction {
	...
	@Method(desc = "新增部门",
			logicStep = "1.设置部门属性信息(非页面传入的值)" +
					"2.新增部门信息")
	@Param(name = "departmentInfo", desc = "Department_info的实体", range = "Department_info的实体", isBean = true)
	...
	public void addDepartmentInfo(Department_info departmentInfo) {
		...
		if (checkDepNameIsRepeat(departmentInfo.getDep_name())) {
			throw new BusinessException("部门名称重复，dep_name=" + departmentInfo.getDep_name());
		}
	...
	@Method(desc = "修改部门信息", logicStep = "1.检查待修改的的部门是否存在，存在则修改")
	@Param(name = "departmentInfo", desc = "Department_info的实体", range = "Department_info的实体",
			example = "departmentInfo", isBean = true)
	@Return(desc = "void 无返回结果", range = "无")
	public void updateDepartmentInfo(Department_info departmentInfo) {
		//数据权限校验：不做权限检查
		//1.检查待修改的的部门信息是否存在，存在则修改
	...
	public boolean checkDepIdIsExist(long dep_id) {
		...
		return Dbo.queryNumber("SELECT COUNT(1) FROM " + Department_info.TableName + " WHERE dep_id = ?", dep_id)
