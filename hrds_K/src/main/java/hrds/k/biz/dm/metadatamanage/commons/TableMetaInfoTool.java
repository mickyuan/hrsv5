...
public class TableMetaInfoTool {
	...
	public static void restoreDCLTableInfo(Dq_failure_table dft, DatabaseWrapper db) {
		...
		if (null != dsr) {
			//获取表存储信息编号
			Table_storage_info tsi = Dbo.queryOneObject(Table_storage_info.class,
				"SELECT * FROM " + Table_storage_info.TableName + " tsi WHERE tsi.table_id=?", dsr.getTable_id())
				.orElseThrow(() -> new BusinessException("获取表存储层信息的SQL执行错误!"));
			//恢复表存储层登记关系
			Dtab_relation_store dtrs = new Dtab_relation_store();
			dtrs.setTab_id(tsi.getStorage_id());
