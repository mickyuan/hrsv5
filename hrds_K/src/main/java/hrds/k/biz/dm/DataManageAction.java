...
public class DataManageAction extends BaseAction {
    ...
    public List<Map<String, Object>> getTableStatistics(int statistics_layer_num) {
        ...
        //设置查询sql
        asmSql.clean();
        asmSql.addSql("select dsl.dsl_id,dsl.dsl_name,count(drt.storage_id) from data_store_layer dsl" +
                " left join data_relation_table drt on dsl.dsl_id=drt.dsl_id group by dsl.dsl_id" +
                " order by count desc");
    ...
    public Map<String, Object> getRuleStatistics() {
        ...
        //检查通过数
        asmSql.clean();
        asmSql.addSql("SELECT COUNT(1) AS check_passes_number FROM dq_result WHERE verify_result = ?")
        ...
        //数据异常数
        asmSql.clean();
        asmSql.addSql("SELECT COUNT(1) AS check_exception_number FROM dq_result WHERE verify_result = ?")
        ...
        //执行失败数
        asmSql.clean();
        asmSql.addSql("SELECT COUNT(1) AS execution_failed_number FROM dq_result WHERE verify_result = ?")
        ...
        //规则总数
        asmSql.clean();
        asmSql.addSql("SELECT COUNT(1) AS rule_total_number FROM dq_result");
        ...
        //检查通过top 5
        asmSql.clean();
        asmSql.addSql("SELECT task_id, verify_date, target_tab FROM dq_result WHERE verify_result = ? " +
