...
//        1: 根据提供的存储层名称查询DB连接信息
		long countNum = Dbo
			.queryNumber("SELECT COUNT(1) FROM " + Data_store_layer.TableName + " Where lower(dsl_name) = lower(?) ",
				storageType).orElseThrow(() -> new BusinessException("SQL查询异常"));
		if (countNum == 0) {
			throw new BusinessException("储存层名称(" + storageType + "),不存在");
		}
		List<Map<String, Object>> databaseInfo = Dbo
			.queryList("SELECT t2.storage_property_key,t2.storage_property_val FROM  " +
				"" + Data_store_layer.TableName
				+ " t1 JOIN "
				+ Data_store_layer_attr.TableName
				+ " t2 on t1.dsl_id = t2.dsl_id where lower(t1.dsl_name) = lower(?)", storageType);
