...
import hrds.commons.entity.Data_mart_info;
...
public class MarketInfoAction extends BaseAction {
	...
	public Result getMarketTakesUpTop3Storage() {
		return Dbo.queryResult("SELECT data_mart_info.data_mart_id,mart_name,sum(soruce_size) source_size FROM" +
						...
						" and data_mart_info.data_mart_id in(select data_mart_id from data_mart_info where create_id =?)" +
						" GROUP BY data_mart_info.data_mart_id,mart_name ORDER BY source_size DESC limit 3 ",
				getUserId());
	}
	...
	public void addMarketInfo(Data_mart_info dataMartInfo) {
		...
		if (StringUtil.isBlank(dataMartInfo.getMart_number())) {
			throw new BusinessException("集市名称不能为空! mart_number=" + dataMartInfo.getMart_number());
		}
		//2.设置集市属性信息
		if (checkMarketNameIsRepeat(dataMartInfo.getMart_name())) {
			throw new BusinessException("集市名称名称重复! mart_name=" + dataMartInfo.getMart_name());
		}
		if (checkMarketNameIsRepeat(dataMartInfo.getMart_name())) {
	...
	public void deleteMarketInfo(long data_mart_id) {
		if (checkExistDataUnderTheMarket(data_mart_id)) {
			throw new BusinessException("集市下还存在数据表!");
		}
		DboExecute.deletesOrThrow("删除集市失败!，data_mart_id=" + data_mart_id,
				"DELETE FROM data_mart_info WHERE data_mart_id = ? ", data_mart_id);
	}
	...
	private boolean checkMarketNameIsRepeat(String mart_name) {
		//1.根据 mart_name 检查集市名称是否重复
		return Dbo.queryNumber("select count(data_mart_id) count from " + Data_mart_info.TableName + " WHERE mart_name =?",
				mart_name).orElseThrow(() -> new BusinessException("检查集市名称否重复的SQL编写错误")) != 0;
	}
	...
	private boolean checkMarketNumberIsRepeat(String mart_number) {
		//1.根据 mart_name 检查集市名称是否重复
		return Dbo.queryNumber("select count(data_mart_id) count from " + Data_mart_info.TableName + " WHERE mart_number =?",
				mart_number).orElseThrow(() -> new BusinessException("检查集市编号否重复的SQL编写错误")) != 0;
	}
	...
	private boolean checkExistDataUnderTheMarket(long data_mart_id) {
